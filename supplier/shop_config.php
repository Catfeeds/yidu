...
if ($_REQUEST['act'] == 'list_edit')
{
	...
	/*$sql="select * from hunuo_supplier_shop_config where code='jwdbj' and supplier_id=".$_SESSION['supplier_id'];
	$jwd=$db->getRow($sql);
	if(empty($jwd['id']))
	{
		$sql="insert into hunuo_supplier_shop_config(parent_id, code,type,sort_order,supplier_id) VALUES ('1','jwdbj','jwd','1','".$_SESSION['supplier_id']."')";
     }   */
...
elseif ($_REQUEST['act'] == 'post')
{
	...
	//$jd= substr($jd,7);
	//$wd= substr($wd,7);
	$sql="select * from hunuo_supplier_shop_config where code='jingdu' and supplier_id=".$_SESSION['supplier_id'];
	$jingdu=$db->getRow($sql);
	if(empty($jingdu['id']))
	{
		$sql="insert into hunuo_supplier_shop_config(parent_id, code,type,value,sort_order,supplier_id) VALUES ('1','jingdu','jwd','".$jd."','1','".$_SESSION['supplier_id']."')";
		@$db->query($sql);
	 }else{
		$sql="update hunuo_supplier_shop_config set value='".$jd."' where code='jingdu' and supplier_id=".$_SESSION['supplier_id'];
		@$db->query($sql);
	 }

	$sql="select * from hunuo_supplier_shop_config where code='weidu' and supplier_id=".$_SESSION['supplier_id'];
	$weidu=$db->getRow($sql);
	if(empty($weidu['id']))
	{
		$sql="insert into hunuo_supplier_shop_config(parent_id, code,type,value,sort_order,supplier_id) VALUES ('1','weidu','jwd','".$wd."','1','".$_SESSION['supplier_id']."')";
		@$db->query($sql);
	 }else{
		$sql="update hunuo_supplier_shop_config set value='".$wd."' where code='weidu' and supplier_id=".$_SESSION['supplier_id'];
		@$db->query($sql);
	 }
    ...
    foreach ($_FILES AS $code => $file)
    {
        ...
        if ((isset($file['error']) && $file['error'] == 0) || (!isset($file['error']) && $file['tmp_name'] != 'none'))
        {
            ...
            else
            {
                ...
                elseif($code == 'shop_banner')
                {
                    $ext = array_pop(explode('.', $file['name']));
                    $file_name =  ROOT_PATH.DATA_DIR.'/supplier/shop_banner/'.'banner_supplier'.$_SESSION['supplier_id'].'.'.$ext;
                    if (file_exists($file_var_list[$code]['value']))
                    {
                        @unlink($file_var_list[$code]['value']);
                    }
                ...
                if (move_upload_file($file['tmp_name'], $file_name))
                {
                	if($code == 'shop_logo'){
						...
						$file_name = '/'.$image->make_thumb($file_name,180,180,$to_path);
                	}
                    if($code == 'shop_banner'){
                        $file_name = '/'.DATA_DIR.'/supplier/shop_banner/'.'banner_supplier'.$_SESSION['supplier_id'].'.'.$ext;
                    }
