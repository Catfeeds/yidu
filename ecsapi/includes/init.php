...
define('ROOT_PATH', str_replace('ecsapi/includes/init.php', '', str_replace('\\', '/', __FILE__)));
define('TOKEN', "qphp");

require_once(ROOT_PATH.'includes/360_safe3.php');
...
require(ROOT_PATH . 'ecsapi/includes/cls_Response.php');
require(ROOT_PATH . 'ecsapi/includes/cls_ActivityException.php');
require(ROOT_PATH . 'ecsapi/config/config_api.php');
require(ROOT_PATH . 'ecsapi/includes/cls_log.php');
require(ROOT_PATH . 'ecsapi/includes/cls_api.php');
try{
    $API=new ApiControl();
}catch(Exception $e){
...
//F:\project\jdy\ecsapi\includes\init.php
 ...
 * 微信支付支付生成预付单
 */
function weixin_pay_order($send_data){

    $rtn = array();
    // 商家API密钥
    $api_key = 'dgtekvydkie5dhc6jdux80jksrdewbxg';
    // 发送地址
    ...
    $rand_str = ''; $len = count($seed);
    for($i = 0; $i<32; $i++){
        $k = rand(0, $len-1);
        $rand_str .= $seed[$k];
    }
    // 随机字符串
    $send_data['nonce_str'] = $rand_str;
    /*** 生成随机字符串 end ***/

    /*** 生成签名 start ***/
    ksort($send_data); $stringA = '';
    foreach($send_data as $i=>$item){
        $stringA .= $i.'='.$item.'&';
    }
    $stringA .= 'key='.$api_key;
