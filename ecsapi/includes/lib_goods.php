        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
            {
                ...
                if (isset($row['cat_id']) != NULL)
                {
                    ...
                    # 关联品牌
                    $cat_arr[$row['cat_id']]['cat_brand'] = get_brand_cat($row['cat_id']);
                    # 关联热词
                    $cat_arr[$row['cat_id']]['hot_terms'] = get_hot_terms($row['cat_id']);
...
function get_brand_cat($cat_id){
    ...
    $sql = 'SELECT cat_brand FROM ' . $GLOBALS['ecs']->table('category') . " WHERE cat_id = '$cat_id' AND is_show = 1 ";
...
function get_hot_terms($cat_id){

    $sql = 'SELECT hot_terms FROM ' . $GLOBALS['ecs']->table('category') . " WHERE cat_id = '$cat_id' AND is_show = 1 ";
    $arr = $GLOBALS['db']->getOne($sql);
 ...
 * @access  public
 * @param   integer     $cat_id     分类编号
 * @param   integer     $type       属性区别  48 男士 49 女士
 * @return  array
 */

function get_categories_tree1($cat_id = 0 , $attr = 49 ,$num = 4)
{
    ...
    /*
     ...
     如果不是取当前分类及其下的子分类
    */
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('category') . " WHERE parent_id = '$parent_id' AND is_show = 1 and FIND_IN_SET( '$attr' , filter_attr )";
    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        ...
        $sql = 'SELECT cat_id,cat_name ,parent_id,is_show ' .
            'FROM ' . $GLOBALS['ecs']->table('category') .
            "WHERE parent_id = '$parent_id' AND is_show = 1 and FIND_IN_SET( '$attr' , filter_attr ) ORDER BY sort_order ASC, cat_id ASC";
        ...
        foreach ($res AS $key => $row)
        {
            ...
            if ($row['is_show'])
            {
                ...
                if (isset($row['cat_id']) != NULL)
                {
                    $cat_arr[$row['cat_id']]['cat_id'] = get_child_tree1($row['cat_id'],$attr);
                }
            }
        }
    }
...
function get_child_tree1($tree_id = 0,$attr1)
{
    $three_arr = array();
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('category') . " WHERE parent_id = '$tree_id' AND is_show = 1 AND FIND_IN_SET ( '$attr1',filter_attr) ";
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0)
    {
        $child_sql = 'SELECT cat_id, cat_name, parent_id, is_show ' .
            'FROM ' . $GLOBALS['ecs']->table('category') .
            "WHERE parent_id = '$tree_id' AND is_show = 1 AND FIND_IN_SET ( '$attr1',filter_attr) ORDER BY sort_order ASC, cat_id ASC";
        $res = $GLOBALS['db']->getAll($child_sql);
        foreach ($res AS $keys => $row)
        {
            ...
            if (isset($row['cat_id']) != NULL)
            {
                $three_arr[$row['cat_id']]['cat_id'] = get_child_tree($row['cat_id'],$attr1);
            }
        }
    }
...
{
    $time = gmtime();
    $sql = 'SELECT g.*, c.measure_unit, b.brand_id, b.brand_name AS goods_brand, m.type_money AS bonus_money, b.brand_desc AS brand_desc , b.brand_logo AS logo , ' .
...
//    echo $sql;die;
    ...
    if ($row !== false)
    {
        ...
        $time = time();
        $today = strtotime(date('Y-m-d', time())); # 0 点时间戳
        $one   = strtotime('+10 hours',$today);
        $two   = strtotime('+12 hours',$today);
        $three = strtotime('+14 hours',$today);
        $four  = strtotime('+16 hours',$today);
        $five  = strtotime('+18 hours',$today);
        $six   = strtotime('+20 hours',$today);
        $sev   = strtotime('+22 hours',$today);
...
//        echo $row['start_time'],$season;die;

        if ($time >= $row['promote_start_date'] && $time <= $row['promote_end_date'] && $row['start_time'] == $season)
        {
 ...
 * @param   string  $start      促销开始日期
 * @param   string  $end        促销结束日期
 * @param   string  $num        属于阶段的促销
 * @return  float   如果还在促销期则返回促销价，否则返回0
 */
function bargain_price1($price, $start, $end, $num)
{
//    echo 1;die;
    if ($price == 0)
    {
    ...
    else
    {
        ...
        if ($time >= $start && $time <= $end && $num == $season)
        {
            return $price;
        }
        else
