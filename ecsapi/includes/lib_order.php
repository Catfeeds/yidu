 ...
 * @return  boolean
 */
function api_addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0,$user_id)
{
    $GLOBALS['err']->clean();
    $_parent_id = $parent;
    $res = array();
    ...
    /* 取得商品信息 */
    $sql = "SELECT g.goods_id,g.goods_name, g.goods_sn, g.is_on_sale, g.is_real, ".
    ...
    {
        $return['res'] = false;
        $return['desc'] = $GLOBALS['_LANG']['goods_not_exists'];
        return $return;
    }
    ...
    if ($parent > 0)
    {
        ...
        if ($GLOBALS['db']->getOne($sql) == 0)
        {
            ...
            $return['res'] = false;
            $return['desc'] = $GLOBALS['_LANG']['no_basic_goods'];
            return $return;
        }
    }
    ...
    if ($goods['is_on_sale'] == 0)
    {
        ...
        $return['res'] = false;
        $return['desc'] = $GLOBALS['_LANG']['not_on_sale'];
        return $return;
    }
    ...
    if (empty($parent) && $goods['is_alone_sale'] == 0)
    {
        ...
        $return['res'] = false;
        $return['desc'] = $GLOBALS['_LANG']['cannt_alone_sale'];
        return $return;
    }
    ...
    if ($GLOBALS['_CFG']['use_storage'] == 1)
    {
        ...
        if ($num > $goods['goods_number'])
        {
            ...
            $res['res'] = false;
            $res['desc'] = sprintf($GLOBALS['_LANG']['shortage']);
            return $res;
        }
    ...
    if ($num > 0)
    {
        ...
        */
        ...
        if(!empty($virtual_sale_id))
        {
            ...
            $GLOBALS['db']->query("update ".$GLOBALS['ecs']->table('cart')." set rec_type=".CART_VIRTUAL_GROUP_GOODS." WHERE user_id = '" .$user_id. "' AND goods_id = '$goods_id' ");   
        }
        
    }

    /* 把赠品删除 */
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') . " WHERE user_id = '" . $user_id . "' AND is_gift <> 0";
    $GLOBALS['db']->query($sql);
 ...
 */
function api_clear_cart($goods_id,$user_id)
{
