...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'selcart')
{
    ...
    $sel_goods = num_str_deal($_GET['sel_goods']);//防注入攻击处理
	if ($sel_goods)
	{
        if($sel_goods){
...
elseif ($_REQUEST['act']=='delAddress')
{
	...
	$consignee_list = get_consignee_list2();
...
elseif ($_REQUEST['act']=='saveAddress')
{
	...
	$address_info = $json->decode($_POST['address']);
	$consignee = array(
            'address_id'    => empty($address_info->address_id) ?  '0'  :   intval($address_info->address_id),
            'consignee'     => empty($address_info->consignee)  ? '' :   compile_str(trim($address_info->consignee)),
            'country'       => empty($address_info->country)    ? '' :   intval($address_info->country),
            'province'      => empty($address_info->province)   ? '' :   intval($address_info->province),
            'city'          => empty($address_info->city)       ? '' :   intval($address_info->city),
            'district'      => empty($address_info->district)   ? '' :   intval($address_info->district),
            'email'         => empty($address_info->email)      ? '' :   compile_str($address_info->email),
            'address'       => empty($address_info->address)    ? '' :   compile_str($address_info->address),
            'zipcode'       => empty($address_info->zipcode)    ? '' :   compile_str(make_semiangle(trim($address_info->zipcode))),
            'tel'           => empty($address_info->tel)        ? '' :   compile_str(make_semiangle(trim($address_info->tel))),
            'mobile'        => empty($address_info->mobile)     ? '' :   compile_str(make_semiangle(trim($address_info->mobile))),
     );
	...
	$result = array('error' => 0, 'message' => '', 'content' => '', 'closediv'=>$address_info->closediv);
	$consignee_list = get_consignee_list2();
	$smarty->assign('consignee_list', $consignee_list);
	$result['content'] =  $smarty->fetch("library/address_list.lbi");
	...
	if ($address_info->shipping_bian=='0' && $address_info->address_id>0 && $address_info->address_id==$_SESSION['flow_consignee']['address_id'])
	{
		$sql = "SELECT * ".
                    " FROM " . $GLOBALS['ecs']->table('user_address') .
                    " WHERE  address_id = '".$address_info->address_id."' ";
		$consignee = $GLOBALS['db']->getRow($sql);
		$_SESSION['flow_consignee'] = $consignee;
	}
	if ($address_info->shipping_bian=='1' || ($address_info->shipping_bian=='0' && $address_info->address_id>0 && $address_info->address_id==$_SESSION['flow_consignee']['address_id']))
	{
	$order = flow_order_info();
	$flow_type = isset($_SESSION['flow_type']) ? intval($_SESSION['flow_type']) : CART_GENERAL_GOODS;
    $region            = array($consignee['country'], $consignee['province'], $consignee['city'], $consignee['district']);
...
function get_consignee_list2()
{
	...
	{
				$consignee_list[$cons_key]['address_short_name'] = $cons_val['consignee']."<br>";
				$consignee_list[$cons_key]['address_short_name'] .=  get_region_info($cons_val['province'])."-";
				$consignee_list[$cons_key]['address_short_name'] .=  get_region_info($cons_val['city'])."-";
				$consignee_list[$cons_key]['address_short_name'] .=  get_region_info($cons_val['district'])."&nbsp;";
				$consignee_list[$cons_key]['address_short_name'] .=  sub_str($cons_val['address'],16);
				$consignee_list[$cons_key]['address_short_name'] .=  $cons_val['zipcode'] ? (",".$cons_val['zipcode']) : "";
				$consignee_list[$cons_key]['address_short_name'] .=  "<br>".($cons_val['tel'] != '--' ? $cons_val['tel'] : $cons_val['mobile']);
...
elseif ($_REQUEST['step'] == 'consignee')
{
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET')
    {
        ...
        if ($_SESSION['user_id'] > 0)
        {
            $consignee_list = get_consignee_list2($_SESSION['user_id']);
...
elseif ($_REQUEST['step'] == 'consignee1')
{
    clear_all_files();
    /*------------------------------------------------------ */
    //-- 收货人信息
...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    if ($db->getOne($sql) == 0)
    {
        show_message($_LANG['no_goods_in_cart'].'666', '', '', 'warning');
    }else{
            if($flow_type != CART_EXCHANGE_GOODS)
		{
		$time_xg_now=gmtime();
...
$consignee['region'] = $db->getOne($sql);
    ...
    if ($_SESSION['user_id'] > 0)
    {
		...
		foreach ($consignee_list  as $cons_key => $cons_val)
		{
			...
			$consignee_list[$cons_key]['address_short_name'] .=  sub_str($cons_val['address'],16);
			$consignee_list[$cons_key]['address_short_name'] .=  $cons_val['zipcode'] ? (",".$cons_val['zipcode']) : "";
			$consignee_list[$cons_key]['address_short_name'] .=  "<br>".$cons_val['tel'];
			if ($consignee['address_id'] == $cons_val['address_id'])
			{
				$consignee_list[$cons_key]['def_addr'] =1;
				$have_def_addr=1;
...
//    $smarty->assign('cod_disabled',    $cod_disabled);
        ...
        && ($flow_type != CART_GROUP_BUY_GOODS && $flow_type != CART_EXCHANGE_GOODS))
    {
        ...
        //wenjun start
           ...
           // print_r($jifen);exit;
            $smarty->assign('allow_use_integral', 1);
            $smarty->assign('order_max_integral', $jifen);
            $smarty->assign('your_integral',      $user_info['pay_points']); // 用户积分
        //wenjun end
    }*/
...
elseif ($_REQUEST['step'] == 'select_shipping')
{
    ...
    {
        $result['error'] = $_LANG['no_goods_in_cart'].'777';
    }
    else
    {
        /* 取得购物流程设置 */
...
elseif ($_REQUEST['step'] == 'select_insure')
{
    ...
    if (empty($cart_goods) || !check_consignee_info($consignee, $flow_type))
    {
        $result['error'] = $_LANG['no_goods_in_cart'].'888';
    }
    else
    {
        /* 取得购物流程设置 */
...
elseif ($_REQUEST['step'] == 'select_payment')
{
    ...
    if (empty($cart_goods) || !check_consignee_info($consignee, $flow_type))
    {
        $result['error'] = $_LANG['no_goods_in_cart'].'999';
    }
    else
    {
        /* 取得购物流程设置 */
...
elseif ($_REQUEST['step'] == 'select_pack')
{
    ...
    if (empty($cart_goods) || !check_consignee_info($consignee, $flow_type))
    {
        $result['error'] = $_LANG['no_goods_in_cart'].'10';
    }
    else
    {
        /* 取得购物流程设置 */
...
elseif ($_REQUEST['step'] == 'select_card')
{
    ...
    if (empty($cart_goods) || !check_consignee_info($consignee, $flow_type))
    {
        $result['error'] = $_LANG['no_goods_in_cart'].'11';
    }
    else
    {
        /* 取得购物流程设置 */
...
{    /*------------------------------------------------------ */
    ...
    else
    {
        ...
        if (empty($cart_goods) || !check_consignee_info($consignee, $flow_type))
        {
            $result['error'] = $_LANG['no_goods_in_cart'].'12';
        }
        else
        {
...
elseif ($_REQUEST['step'] == 'change_integral')
{
    ...
    if (count($flow_points) > 1) {
        foreach ($flow_points as $key => $value) {
            $order_max_integral += $value;
        }
    } else {
        $order_max_integral = $flow_points[$suppid];
    }
    // henson end
    ...
    elseif ($points > $order_max_integral)
    {
        $result['error'] = sprintf($_LANG['integral_too_much'], $order_max_integral);
    }
    else
    {
        /* 取得购物类型 */
        ...
        if (empty($cart_goods) || !check_consignee_info($consignee, $flow_type))
        {
            $result['error'] = $_LANG['no_goods_in_cart'].'13';
        }
        else
        {
...
elseif ($_REQUEST['step'] == 'change_bonus')
{
    ...
    if (empty($cart_goods) || !check_consignee_info($consignee, $flow_type))
    {
        $result['error'] = $_LANG['no_goods_in_cart'].'14';
    }
    else
    {
        /* 取得购物流程设置 */
...
elseif ($_REQUEST['step'] == 'change_needinv')
{
    ...
    if (empty($cart_goods) || !check_consignee_info($consignee, $flow_type))
    {
        $result['error'] = $_LANG['no_goods_in_cart'].'15';
        die($json->encode($result));
    }
    else
    {
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    if ($db->getOne($sql) == 0)
    {
        show_message($_LANG['no_goods_in_cart'].'111', '', '', 'warning');
    }
    ...
    if (empty($cart_goods))
    {
        show_message($_LANG['no_goods_in_cart'].'222', $_LANG['back_home'], './', 'warning');
    }
    ...
    foreach ($cart_goods_new as $ckey=>$cval){
		...
		$order['defaultbank'] = $_POST['hunuo_bank'] ? trim($_POST['hunuo_bank']) : "";
    ...
    foreach($order_info as $ok=>$order){
        ...
        // 会员季度消费记录id
        $order['consume_id'] = get_quarter_consume($order['user_id']);
	    ...
	    $sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
	                ...
	                "goods_price,split_money, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id, package_attr_id, is_recommend) ".
...
//                    }
    ...
    if ($order['supplier_id'] > 0) {
        bind_fands($order['supplier_id'], $order['user_id']);
    }
}
...
elseif ($_REQUEST['step'] == 'done1')
{
    ...
    if ($db->getOne($sql) == 0)
    {
        show_message($_LANG['no_goods_in_cart'].'333', '', '', 'warning');
    }
    $sql = "SELECT * FROM ".$ecs->table('cart')."WHERE $sql_where AND parent_id = 0 AND is_gift > 0 AND rec_type = '$flow_type'";
    $res = $db->getAll($sql);
    foreach($res as $key=>$value)
    ...
    {
        /* 如果不完整则转向到收货人信息填写界面 */
        ecs_header("Location: flow.php?step=consignee&ex=1\n");
//        show_message('请先添加收货地址！');
        exit;
    }
    ...
    if (empty($cart_goods))
    {
        show_message($_LANG['no_goods_in_cart'].'444', $_LANG['back_home'], './', 'warning');
    }
    ...
    foreach ($cart_goods_new as $ckey=>$cval){
        ...
        $order = array(
            //'shipping_id'     => intval($_POST['shipping']),
            'pay_id'          => intval($_SESSION['payment']),
            'pack_id'         => isset($_POST['pack']) ? intval($_POST['pack']) : 0,
            'card_id'         => isset($_POST['card']) ? intval($_POST['card']) : 0,
            'card_message'    => trim($_POST['card_message']),
            'surplus'         => $order_surplus,//isset($order_surplus[$ckey]) ? floatval($order_surplus[$ckey]) : 0.00,
            ...
            // 2018 wenjun start
            'supplier_id'     => intval($_SESSION['supplier_id']),
            // 2018 wenjun end
        ...
        // 配送方式
            ...
            $shipid = $db->getOne("select shipping_id from ".$ecs->table('shipping')." where is_default_show=1 and supplier_id=0");
        ...
        //配送方式的钱算到里面
        $order['shipping_pay'][$ckey] = 0;

        /* 订单中的总额 */
        $total = order_fee($order, $cart_goods, $consignee);
        $order['integral'] = $total['goods_price'];
    ...
    foreach($order_info as $ok=>$order){
        ...
        //给用户的抽奖序列号
        $order['extension_num'] = $_SESSION['extension_num'];
        ...
        $db->query($sql);
         ...
         //改变参与抽奖人数
        $num = $db->getOne("SELECT exchange_number "." FROM " .$GLOBALS['ecs']->table('exchange_goods') . " WHERE goods_id = $order[extension_id] ");

        $exchange_number = $num*1 + $_SESSION['num']*1 ;
        $sql = "UPDATE ". $ecs->table('exchange_goods') ." SET exchange_number = $exchange_number WHERE goods_id=".$order['extension_id'];
        $db->query($sql);
...
//                    }
    ...
    if ($order['order_amount'] > 0)
    {
        ...
        if ($order['pay_id'] == 1) {
            /* 进入支付页面 */
            if ($is_app == 1) {
                ecs_header("Location: ./apppay/pay.php?out_trade_no=$order[log_id]&pay_code=alipay&flag=1\n");
            } else {
                ecs_header("Location: ./pay/alipayapi.php?out_trade_no=$order[log_id]&total_fee=$order[order_amount]\n");
            }
            exit;
        }
        ...
        if ($order['pay_id'] == 7) {
             /* 进入支付页面 */
             if ($is_app == 1) {
                 ecs_header("Location: ./apppay/pay.php?out_trade_no=$order[log_id]&pay_code=weixin&flag=1\n");
             } else {

                 ecs_header("Location: ./weixinpay.php?out_trade_no=$order[log_id]\n");
             }
             exit;
        }
...
//    die(sprintf($_LANG['bonus_is_ok'], price_format($bonus['type_money'], false)));
    ...
    if (empty($cart_goods) || !check_consignee_info($consignee, $flow_type))
    {
        $result['error'] = $_LANG['no_goods_in_cart'].'555';
    }
    else
    {
        /* 取得购物流程设置 */
