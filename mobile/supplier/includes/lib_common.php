...
function Recordkeyword($word_hunuo, $items = 0, $searchengine = 'ecshop')
{
	if(strlen($word_hunuo) < 3 || strlen($word_hunuo) > 30 || strpos($word_hunuo, ' ') !== false) return;
	$sql_hunuo = "SELECT * FROM " .$GLOBALS['ecs']->table('keyword'). " WHERE searchengine='ecshop' AND word='$word_hunuo'";
	$r = $GLOBALS['db']->getRow($sql_hunuo);
	if($r)
	{
		$items = intval($items) ;
        ...
        $sql_hunuo = "UPDATE " . $GLOBALS['ecs']->table('keyword') . " SET " .
                "items = '$items', " .
                "updatetime = '".gmtime()."', " .
				...
				"WHERE w_id = '".$r['w_id']."'";
		$GLOBALS['db']->query($sql_hunuo);
		$w_id = $r['w_id'];
	}
	else
	{
		$letter_hunuo   = gb2py($word_hunuo);
		$sql_hunuo = "INSERT INTO " . $GLOBALS['ecs']->table('keyword') . " (searchengine, word, keyword, letter, items, updatetime, total_search, ".
		"month_search, week_search, today_search, status) " .
                " VALUES ('$searchengine', '$word_hunuo', '$word_hunuo', '$letter_hunuo', '$items', '".gmtime()."', '1', '1', '1', '1', '1')";
        $GLOBALS['db']->query($sql_hunuo);
	...
	{
    	$ip_hunuo       = real_ip();
    	$area_hunuo     = ecs_geoip($ip);
    	$sql_hunuo = 'INSERT INTO ' . $GLOBALS['ecs']->table('keyword_area') . ' ( ' .
                'w_id, access_time, ip_address, area) VALUES (' .
                "'$w_id', '".gmtime()."', '$ip_hunuo', '$area_hunuo')";
    	$GLOBALS['db']->query($sql_hunuo);
	}
}
/**
