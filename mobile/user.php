...
{
	if(strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') && $action !== 'openshop_index')
	{
...
// print_r($user_id);exit;
/* 会员季度消费分红，判断本季度是否能分红 henson */
user_quarter_consume_bonus();
...
$not_login_arr = array(
	'login','act_login','act_edit_password','get_password','send_pwd_email','password','signin','add_tag','collect','return_to_cart','logout','email_list','validate_email','send_hash_mail','order_query','is_registered','check_email','clear_history','qpassword_name','get_passwd_question','check_answer','oath','oath_login','other_login','getverifycode','act_forget_pass','re_pass','open_surplus_password','close_surplus_password','check_register','book_goods','openshop_index'
);

/* 显示页面的action列表 */
$ui_arr = array(
	//wenjun start                    正下面添加了'ex_order_detail'                //wenjun end
	'login','profile','order_list','order_detail','ex_order_detail','address_list','collection_list','message_list','tag_list',
       ...
       'supplier_reg','comment_order','address','set_address','follow_shop','account_manage','my_comment','shaidan_send','agent_apply_one','agent_apply_two','agent_apply_three','agent_apply','agent_index','agent_shop_list','agent_shop_chuzu','shop_chuzu','agent_cx_shop','shop_index','shop_zuyue','openshop_apply','openshop_insert','pay_points_detail'
);
...
function action_default()
{
	...
	$stock_right = $GLOBALS['db']->getOne("SELECT stock_right FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_id = " . $user_id);
	$smarty->assign('stock_right', intval($stock_right));
...
function action_update_head()
{
    ...
    if($_FILES['headimg']['error'] === 0){
		...
		$img_url = filter_var_array($_FILES['headimg'], FILTER_SANITIZE_SPECIAL_CHARS);
		$headimg_original = $image->upload_image($img_url, 'headimg/' . date('Ym'));
		$thumb_path = get_pc_url().'/'.DATA_DIR . '/headimg/' . date('Ym') . '/';
		$headimg_thumb = $image->make_thumb($headimg_original, '80', '80', $thumb_path);
		$headimg_thumb = $headimg_thumb ? $headimg_thumb : $headimg_original;
		$headimg = $GLOBALS['db']->getOne("select headimg from " . $GLOBALS['ecs']->table('users') . " where user_id = $user_id");
		$sql = 'UPDATE ' . $GLOBALS['ecs']->table('users') . " SET `headimg`='$headimg_thumb'  WHERE `user_id`='" . $_SESSION['user_id'] . "'";
		$GLOBALS['db']->query($sql);
		@unlink($headimg);//删除旧头像
		@unlink(ROOT_PATH.'/'.$headimg_original);//删除大图
    }
    echo '1';
}
...
//
	...
	//判断是否抽奖订单
	if (empty($_GET['ex'])) {
		$ex_where .= " AND extension_code != 'exchange_goods' " ;
	}else{
		$ex_where .= " AND extension_code = 'exchange_goods' " ;
		$smarty->assign('ex', 1);
	}
	...
	if (empty($_GET['ex'])) {
		...
		$smarty->display('ex_order_list.dwt');
	}
	//wenjun end
}
...
if (empty($_REQUEST['ex'])) {
	...
	{
		case 101:
			$where .= " and o.extension_code <> 'virtual_good' AND is_lucky = 0 AND pay_status = 2";
			break;
		case 105:
			$where .= " and o.extension_code <> 'virtual_good' AND is_lucky = 1";
			break;
		case 103:
			$where .= " and o.extension_code <> 'virtual_good' AND is_lucky = -1";
			break;
...
//wenjun end
    ...
    if ($_REQUEST['ex']) {
        foreach($orders as $key=>$val){
            ...
            $result[]['info']  = $GLOBALS['smarty']->fetch('library/ex_order_list.lbi');
        }
    }else{
...
function action_back_order_act ()
{
	...
	$imgs = $goods_img_arr ? implode(',', $goods_img_arr) : '';
	...
	if($have_tuikuan)
	{
		...
		$db->query($sql);*/
		$sql = "update " . $ecs->table('back_order') . " set refund_money_1= '$price_refund_all' where back_id='$back_id' ";
		$db->query($sql);
	}
	else
	{
	...
	exit();*/
	$smarty->display('back_apply_ok.dwt');
}
...
//	}
        ...
        //wenjun 08-06 start
		$sql = "SELECT delivery_id,shipping_name,invoice_no  FROM ". $GLOBALS['ecs']->table('delivery_order'). " WHERE order_id = '$order_id'AND user_id = '$user_id'";
        $wuliu1 = $GLOBALS['db']->getAll($sql);
        $kuaidi_list = array();
        foreach($wuliu1 as $key=>$value){
            $wuliu = array('EMS 国内邮政特快专递'=>'ems','EMS'=>'ems','中国邮政'=>'ems','申通快递'=>'shentong',"圆通速递"=>'yuantong','顺丰速运'=>'shunfeng','天天快递'=>'tiantian','韵达快递'=>'yunda','中通速递'=>'zhongtong','龙邦物流'=>'longbanwuliu','宅急送'=>'zhaijisong','全一快递'=>'quanyikuaidi','汇通速递'=>'huitongkuaidi','民航快递'=>'minghangkuaidi','亚风速递'=>'yafengsudi','快捷速递'=>'kuaijiesudi','华宇物流'=>'tiandihuayu','中铁快运'=>'zhongtiewuliu','百世汇通'=>'huitongkuaidi','全峰快递'=>'quanfengkuaidi','德邦'=>'debangwuliu','FedEx'=>'fedex','UPS'=>'ups','DHL'=>'dhl');
			$wuliu_state = array('在途','揽件','疑难','签收','退签','派件','退回');
			$typeCom = $value['shipping_name'];
			
			if (is_numeric($value['invoice_no'])) {
				$typeNu = $value['invoice_no'];
			}else{
				$typeNu = '';
			}
			if (!empty($typeCom) && !empty($typeNu)) {
				...
				$kuaidi_list = $arr['data'];
			}
        }
        //wenjun end
        // print_r($kuaidi_list);exit;
...
//wenjun start
/* 查看抽奖订单详情 */
function action_ex_order_detail()
{
	...
	if($order['order_amount'] > 0 && $order['pay_status'] == PS_UNPAYED && $order['shipping_status'] == SS_UNSHIPPED)
	{
        ...
        $handler = get_ex_order_handler($order);
;
	/* 订单 支付 配送 状态语言项 */
	$order['order_status'] = $_LANG['os'][$order['order_status']];
	$order['pay_status'] = $_LANG['ps'][$order['pay_status']];
...
//wenjun start
	if ($_GET['ex']) {
		//抽奖订单，不用返回积分和添加库存
		$ex = $_GET['ex'];
		if(cancel_order($order_id, $user_id,$ex))
		{
			//计算这个取消订单里面的抽奖码。
			$num = $db->getRow("SELECT extension_id,extension_num FROM " . $ecs->table('order_info') . " WHERE order_id='$order_id'");
			$arr = count(explode(',', $num['extension_num']));

			//查询这个抽奖订单的抽奖商品。更改参与人数
			$exchange_number = $db->getOne("SELECT exchange_number FROM " . $ecs->table('exchange_goods') . " WHERE goods_id='$num[extension_id]'");
			$all = $exchange_number *1 - $arr *1 ;

			$db->query('UPDATE ' . $ecs->table('exchange_goods') . "SET exchange_number = $all WHERE goods_id='$num[extension_id]'");

			ecs_header("Location: user.php?act=order_list&ex=1\n");
			exit();
		}
		else
		{
			$GLOBALS['err']->show($_LANG['order_list_lnk'], 'user.php?act=order_list&ex=1');
		}
	}else{
...
//wenjun start
/* 自动取消抽奖订单 */
function action_ex_cancel_order()
{
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	...
	$order_id = isset($_GET['order_id']) ? intval($_GET['order_id']) : 0;
	$ex = 1 ;//抽奖订单，不用返回积分和添加库存
	$kk = cancel_order($order_id, $user_id,$ex);
	if ($kk) {
		//计算这个取消订单里面的抽奖码。
		$num = $db->getRow("SELECT extension_id,extension_num FROM " . $ecs->table('order_info') . " WHERE order_id='$order_id'");
		$arr = count(explode(',', $num['extension_num']));
...
function action_account_raply()
{
	...
	$ecs = $GLOBALS['ecs'];
	$user_id = $GLOBALS['user_id'];
	$bank_card_id = intval($_REQUEST['id']);
	if ($bank_card_id) {
		$sql = "SELECT a.*, b.bank_name, b.icon FROM " . $ecs->table('user_bank') . " a LEFT JOIN " . $ecs->table('bank_list') . " b ON a.bank_id = b.bank_id WHERE a.user_id = $user_id AND id = $bank_card_id";
	} else {
		$sql = "SELECT a.*, b.bank_name, b.icon FROM " . $ecs->table('user_bank') . " a LEFT JOIN " . $ecs->table('bank_list') . " b ON a.bank_id = b.bank_id WHERE a.user_id = $user_id ORDER BY add_time DESC LIMIT 1";
	}
	$bank_card = $db->getRow($sql);
	$bank_card['bank_num'] = substr($bank_card['bank_num'],-4);

	$smarty->assign('user_info', get_user_info());
	$smarty->assign('bank_card', $bank_card);
	$smarty->display('tx_apply.dwt');
}

/* 选择银行卡界面 */
function action_select_bank() {
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	$smarty = $GLOBALS['smarty'];
	...
	$sql = "SELECT a.*, b.bank_name, b.icon FROM " . $ecs->table('user_bank') . " a LEFT JOIN " . $ecs->table('bank_list') . " b ON a.bank_id = b.bank_id WHERE user_id = $user_id";
	$bank_list = $db->getAll($sql);
	if (is_array($bank_list)) {
		foreach ($bank_list as $key => $value) {
			$bank_list[$key]['bank_num'] = substr($value['bank_num'],-4);
		}
	}
	$smarty->assign('bank_list', $bank_list);

    $smarty->display('select_bank.dwt');
}

/* 会员提现申请界面 */
function action_account_raply2()
{
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
...
/* 会员消费积分明细 */
function action_pay_points_detail()
{
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
...
//	print_r($pager);exit;
	// 获取剩余消费积分
	$pay_points = get_user_pay_points($user_id);
	if(empty($pay_points))
	{
		$pay_points = 0;
	}
...
//	print_r($account_log);exit;
	// 模板赋值
	$smarty->assign('pay_points', abs($pay_points));
...
function action_act_account()
{
	...
	{
		$bank_card_id = intval($_REQUEST['bank_card_id']);
		...
		$bank_card = $db->getRow($sql);
		if (empty($bank_card)) {
			show_message('选择的银行卡不存在');
		}

		$surplus['user_note'] = "银行名称：$bank_card[bank_name]，银行卡号：$bank_card[bank_num]，开户支行：$bank_card[bank_branch]，姓名：$bank_card[bank_user]";
	...
	else
	{
		...
		if (! strpos ( $_SERVER ['HTTP_USER_AGENT'], 'MicroMessenger' )) {
            ...
            if($payment_info['pay_name'] == '支付宝'){
            	if ($is_app == 1) {
            		$payment_info['pay_button'] = '<div style="text-align:center"><input type="button" value="使用支付宝支付" onclick="window.location.href=\'./apppay/pay.php?out_trade_no='.$order['log_id'].'&pay_code=alipay&flag=3\'"/></div>';
            	} else {
            ...
            if($payment_info['pay_name'] == '微信支付'){
            	if ($is_app == 1) {
            		$payment_info['pay_button'] = '<div style="text-align:center"><input type="button" onclick="window.location.href=\'./apppay/pay.php?out_trade_no='.$order['order_id'].'&pay_code=weixin&flag=3\'"/></div>';
            	} else {
...
function action_check_surplus_open()
{
	...
	$sql = 'SELECT `is_surplus_open` FROM ' . $GLOBALS['ecs']->table('users') . ' WHERE `user_id` = \'' . $_SESSION['user_id'] . '\'' . 'LIMIT 1';
...
function action_verify_surplus_password()
{
	...
	$sql = 'SELECT COUNT( * ) FROM ' . $GLOBALS['ecs']->table('users') . ' WHERE `user_id` = \'' . $_SESSION['user_id'] . '\'' . 'AND `surplus_password` = \'' . md5($_GET['surplus_password']) . '\'';
	$count = $GLOBALS['db']->getOne($sql);
	echo $count;
	exit();
}
...
function action_agent_apply_one() {
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	$smarty = $GLOBALS['smarty'];
	...
	// 是否已申请
	$is_apply = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('agent_apply') . " WHERE is_pass = 0 AND user_id = " . $user_id);
	if ($is_apply > 0) {
		show_message('您已申请过，请等耐心等待审核');
	}

	// 业主不能重复申请
	$is_agent = $GLOBALS['db']->getOne("SELECT is_agent FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_id = " . $user_id);
	if ($is_agent) {
		show_message('您已经是业主，不能重复申请');
	}

	$smarty->assign('agent_shop_cost_c', $_CFG['agent_shop_cost_c']);	//创业型业主店铺管理费
	$smarty->assign('agent_shop_cost_x', $_CFG['agent_shop_cost_x']);	//小型业主店铺管理费
	$smarty->assign('agent_shop_cost_z', $_CFG['agent_shop_cost_z']);	//中型业主店铺管理费
	$smarty->assign('agent_shop_cost_d', $_CFG['agent_shop_cost_d']);	//大型业主店铺管理费
	$smarty->assign('agent_shop_cost_v', $_CFG['agent_shop_cost_v']);	//VIP业主店铺管理费

	$smarty->assign('agent_lr_c', $_CFG['shop_renter_cost'] - $_CFG['agent_shop_cost_c']);	//创业型业主利润
	$smarty->assign('agent_lr_x', $_CFG['shop_renter_cost'] - $_CFG['agent_shop_cost_x']);	//小型业主利润
	$smarty->assign('agent_lr_z', $_CFG['shop_renter_cost'] - $_CFG['agent_shop_cost_z']);	//中型业主利润
	$smarty->assign('agent_lr_d', $_CFG['shop_renter_cost'] - $_CFG['agent_shop_cost_d']);	//大型业主利润
	$smarty->assign('agent_lr_v', $_CFG['shop_renter_cost'] - $_CFG['agent_shop_cost_v']);	//VIP业主利润

	$smarty->assign('agent_shop_num_c', $_CFG['agent_shop_num_c']);	//创业型业主店铺数量
	$smarty->assign('agent_shop_num_x', $_CFG['agent_shop_num_x']);	//小型业主店铺数量
	$smarty->assign('agent_shop_num_z', $_CFG['agent_shop_num_z']);	//中型业主店铺数量
	$smarty->assign('agent_shop_num_d', $_CFG['agent_shop_num_d']);	//大型业主店铺数量
	$smarty->assign('agent_shop_num_v', $_CFG['agent_shop_num_v']);	//VIP业主店铺数量

	$smarty->assign('agent_zs_cost_c', $_CFG['agent_zs_cost_c']);	//创业型业主终身业主费
	$smarty->assign('agent_zs_cost_x', $_CFG['agent_zs_cost_x']);	//小型业主终身业主费
	$smarty->assign('agent_zs_cost_z', $_CFG['agent_zs_cost_z']);	//中型业主终身业主费
	$smarty->assign('agent_zs_cost_d', $_CFG['agent_zs_cost_d']);	//大型业主终身业主费
	$smarty->assign('agent_zs_cost_v', $_CFG['agent_zs_cost_v']);	//VIP业主终身业主费

	$smarty->display('agent_apply_1.dwt');
}

function action_agent_apply_two() {
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	$smarty = $GLOBALS['smarty'];
	...
	$smarty->display('agent_apply_2.dwt');
}

function action_agent_apply_three() {
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	$smarty = $GLOBALS['smarty'];
	...
	$agent_type = intval($_REQUEST['agent_type']);
	$smarty->assign('agent_type', $agent_type);

	$smarty->display('agent_apply_3.dwt');
}

function action_check_agent_code() {
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	$smarty = $GLOBALS['smarty'];
	...
	$parent_agent_code = trim($_REQUEST['parent_agent_code']);
	$res = array();
	if ($parent_agent_code) {
		$agent_exist = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('users') . " WHERE agent_code = '" . $parent_agent_code . "'");
...
function action_agent_apply() {
    ...
    $sql = "insert into " . $GLOBALS['ecs']->table('agent_apply') . "(`name`,`parent_agent_code`,`phone`,`province`,`city`,`district`,`address`,`card`,`face_card`,`back_card`,`is_pass`,`agent_type`,`user_id`,`add_time`) values('$name','$parent_agent_code','$phone','$province','$city','$district','$address','$card','$face_card','$back_card','0','$agent_type','$user_id','" . gmtime() . "')";
    $GLOBALS['db']->query($sql);
    $smarty->assign('page_title', '申请业主'); // 页面标题
    $smarty->display('apply_ok.dwt');
}

/**
 * 业主管理页面
 */
function action_agent_index() {
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	$smarty = $GLOBALS['smarty'];
	...
	// 未转出
	$wait_chuzu = $db->getOne("select count(*) from " . $ecs->table('agent_shop') . " where chuzu_status = 0 and agent_id = $user_id");
	$smarty->assign('wait_chuzu', $wait_chuzu);

	// 待激活
	$wait_jh = $db->getOne("select count(*) from " . $ecs->table('agent_shop') . " where chuzu_status = 1 and agent_id = $user_id");
	$smarty->assign('wait_jh', $wait_jh);

	// 已激活
	$has_chuzu = $db->getOne("select count(*) from " . $ecs->table('agent_shop') . " where chuzu_status = 2 and agent_id = $user_id");
	$smarty->assign('has_chuzu', $has_chuzu);

	// 已过期
	$has_guoqi = $db->getOne("select count(*) from " . $ecs->table('agent_shop') . " where chuzu_status = 3 and agent_id = $user_id");
	$smarty->assign('has_guoqi', $has_guoqi);

	// 店铺会员
	$shop_member_num = $db->getOne("select count(*) from " . $ecs->table('agent_shop') . " where user_id > 0 and agent_id = $user_id");
	$smarty->assign('shop_member_num', $shop_member_num);

	// 未安置会员
	$weianzhi_member_num = $db->getOne("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('users') . " a LEFT JOIN " . $GLOBALS['ecs']->table('agent_shop') . " b ON a.parent_id = b.user_id WHERE b.chuzu_status = 3 AND b.agent_id = " . $user_id);
	$smarty->assign('weianzhi_member_num', $weianzhi_member_num);

    $smarty->display('agent_index.dwt');
}

/**
 * 业主店铺列表
 */
function action_agent_shop_list() {
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	$smarty = $GLOBALS['smarty'];
	...
	$chuzu_status = intval($_REQUEST['chuzu_status']);	//0未出租 1待激活 2已出租 3已过期

	switch ($chuzu_status) {
		case 0:	//未转出
			$sql = "select count(*) from " . $ecs->table('agent_shop') . " where agent_id = " . $user_id . " and chuzu_status = " . $chuzu_status;
			...
			$sql = "select * from " . $ecs->table('agent_shop') . " where agent_id = " . $user_id . " and chuzu_status = " . $chuzu_status . " order by add_time desc limit " . $pager['start'] . ", " . $pager['size'];
			$shop_list = $db->getAll($sql);
			foreach ($shop_list as $key => $value) {
				$shop_list[$key]['add_time'] = local_date('Y-m-d', $value['add_time']);
			}
			break;
		case 1:	//待激活
			$sql = "select count(*) from " . $ecs->table('agent_shop') . " a left join " . $ecs->table('supplier') . " b on a.user_id = b.user_id where a.agent_id = " . $user_id . " and a.chuzu_status = " . $chuzu_status;
			...
			$sql = "select a.*, b.contacts_name, b.contacts_phone from " . $ecs->table('agent_shop') . " a left join " . $ecs->table('supplier') . " b on a.user_id = b.user_id where a.agent_id = " . $user_id . " and a.chuzu_status = " . $chuzu_status . " order by add_time desc limit " . $pager['start'] . ", " . $pager['size'];

			$shop_list = $db->getAll($sql);
			foreach ($shop_list as $key => $value) {
				$shop_list[$key]['chuzu_time'] = local_date('Y-m-d', $value['chuzu_time']);
			}
			break;
		case 2:	//已转出
			$sql = "select count(*) from " . $ecs->table('agent_shop') . " a left join " . $ecs->table('supplier') . " b on a.user_id = b.user_id where a.agent_id = " . $user_id . " and a.chuzu_status = " . $chuzu_status;
			...
			$shop_list = $db->getAll($sql);
			foreach ($shop_list as $key => $value) {
				$shop_list[$key]['jh_time'] = local_date('Y-m-d', $value['jh_time']);
			}
			break;
		case 3:	//已过期
			$sql = "select count(*) from " . $ecs->table('agent_shop') . " a left join " . $ecs->table('supplier') . " b on a.user_id = b.user_id where a.agent_id = " . $user_id . " and a.chuzu_status = " . $chuzu_status;
			...
			$shop_list = $db->getAll($sql);
			foreach ($shop_list as $key => $value) {
				$shop_list[$key]['end_time'] = local_date('Y-m-d', $value['end_time']);
			}
			break;
		default:
			break;
	...
	$smarty->assign('pager', $pager);
	$smarty->display('agent_shop_list_' . $chuzu_status . '.dwt');
}

/**
 * 业主转出店铺
 */
function action_agent_shop_chuzu() {
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	$smarty = $GLOBALS['smarty'];
	...
	$sql = "select shop_code from " . $GLOBALS['ecs']->table('agent_shop') . " where shop_id = " . $shop_id;
	$shop_code = $GLOBALS['db']->getOne($sql);

	$smarty->assign('shop_id', $shop_id);
	$smarty->assign('shop_code', $shop_code);
	$smarty->assign('user_id', $user_id);
	$smarty->assign('chuzu_time', local_date('Y-m-d'));

    $smarty->display('agent_shop_chuzu.dwt');
}

/**
 * 业主转出店铺提交
 */
function action_shop_chuzu() {
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	$smarty = $GLOBALS['smarty'];
	...
	$shop_id = intval($_REQUEST['shop_id']);	//店铺ID
	$shop_name = trim($_REQUEST['shop_name']);	//店铺名称
	$name = trim($_REQUEST['name']);	//联系人姓名
	$phone = trim($_REQUEST['phone']);	//手机号
	$password = trim($_REQUEST['password']);
	$confirm_password = trim($_REQUEST['confirm_password']);
	$agent_id = intval($_REQUEST['agent_id']);	//业主ID
	$shop_code = trim($_REQUEST['shop_code']);	//店铺编码

	if ($password !== $confirm_password) {
		show_message('两次密码输入不一致', $_LANG['back_page_up'], '', 'error');
	}

	/*创建用户账号*/
	$register_type == "mobile";
	$mobile_phone = $phone;
	$phone_exist = $db->getOne("select count(*) from " . $ecs->table('users') . " where mobile_phone = '" . $phone . "'");
	if ($phone_exist) {
		// 账号已经存在，如果是普通会员，则升级为店主
		$user_count = $db->getOne("SELECT COUNT(*) FROM " . $ecs->table('users') . " WHERE mobile_phone = '$phone'");
		if ($user_count > 1) {
			show_message('该手机号绑定了多个用户', $_LANG['back_page_up'], '', 'error');
		} else {
			$shop_user_info = $db->getRow("SELECT user_id, is_agent, shop_id  FROM " . $ecs->table('users') . " WHERE mobile_phone = '$phone' LIMIT 1");
			if (empty($shop_user_info['is_agent']) && empty($shop_user_info['shop_id'])) {
				// 设为审核通过
				$sql = "UPDATE " . $ecs->table('users') . " SET `status`='1', `shop_id`=" . $shop_id . " WHERE `user_id`='" . $shop_user_info['user_id'] . "'";
				$db->query($sql);

				/*生成店铺*/
				$save['supplier_id'] = $shop_id;
				$save['user_id'] = $shop_user_info['user_id'];
				$save['company_name'] = $shop_name;
				$save['contacts_name'] = $name;
				$save['contacts_phone'] = $phone;
				$save['tel'] = $phone;
				$save['applynum'] = 3;
				$save['supplier_name'] = $shop_name;
				$save['supplier_rebate'] = 6;	//平台分成百分比6为6%
				$save['status'] = 0;	//默认为待审核
				$save['rank_id'] = 3;	//默认为高级店铺
				$save['agent_id'] = $agent_id;
				$save['shop_code'] = $shop_code;
				$save['shop_id'] = $shop_id;
				$save['add_time'] = gmtime();
				$db->autoExecute($ecs->table('supplier'), $save);
				...
				//添加店铺管理员，默认跟店主前台登录账号同步
				$insql = "INSERT INTO " . $ecs->table('supplier_admin_user') . " (`uid`, `user_name`, `email`, `password`, `ec_salt`, `add_time`, `last_login`, `last_ip`, `action_list`, `nav_list`, `lang_type`, `agency_id`, `supplier_id`, `todolist`, `role_id`, `checked`, `pwd`) ".
				    "VALUES(".$supplier_old['user_id'].", '".$supplier_old['user_name']."', '".$supplier_old['email']."', '".$supplier_old['password']."', '".$supplier_old['ec_salt']."', ".$supplier_old['last_login'].", ".$supplier_old['last_login'].", '".$supplier_old['last_ip']."', 'all', '', '', 0, ".$supplier_old['supplier_id'].", NULL, NULL, 1, $password)";
				$db->query($insql);
				...
				// 设置店铺名称
				$sql = "UPDATE " . $ecs->table('supplier_shop_config') . " SET `value` = '" . $shop_name . "' WHERE code = 'shop_name' and `supplier_id` = " . $supplier_id;
				$db->query($sql);

				// 添加到店铺街（前台显示店铺列表）
				$ssdata['supplier_id'] = $supplier_id;
				$ssdata['supplier_name'] = $shop_name;
				$ssdata['supplier_title'] = $shop_name;
				$ssdata['supplier_desc'] = $shop_name;
				$ssdata['addtime'] = gmtime();
				$ssdata['supplier_type'] = 0;
				$ssdata['supplier_tags'] = '';
				$ssdata['is_show'] = 0;	//默认不显示
				$ssdata['status'] = 0;	//默认为待审核
				...
				$sql = "UPDATE " . $ecs->table('agent_shop') . " SET `user_id`= " . $shop_user_info['user_id'] . ", `chuzu_status` = 1, `chuzu_time` = '" . gmtime() . "', `end_time` = '" . $end_time . "' WHERE `shop_id`= " . $shop_id;
				$db->query($sql);

				// $smarty->assign('username', $username); //原本的
				//wj 修改 08-04
				$smarty->assign('username', $supplier_old['user_name']);
				$smarty->display('agent_shop_chuzu_ok.dwt');
				// header("location:user.php?act=agent_shop_list&chuzu_status=1");
				exit;
			} else {
				show_message('该手机号已经是店主或业主', $_LANG['back_page_up'], '', 'error');
			}
		}
	}
	$username = generate_username_by_mobile($mobile_phone);
	$result = register_by_mobile($username, $password, $mobile_phone);	//创建成功会保存店主用户ID到session
	if ($result) {
		// 设为审核通过
		$sql = "UPDATE " . $ecs->table('users') . " SET `status`='1', `shop_id`=" . $shop_id . " WHERE `user_id`='" . $_SESSION['user_id'] . "'";
		$db->query($sql);

		/*生成店铺*/
		$save['supplier_id'] = $shop_id;
		$save['user_id'] = $_SESSION['user_id'];
		$save['company_name'] = $shop_name;
		$save['contacts_name'] = $name;
		$save['contacts_phone'] = $phone;
		$save['tel'] = $phone;
		...
		// 设置店铺为已转出待激活状态
		$end_time = local_strtotime('+3 month');	//到期时间
		$sql = "UPDATE " . $ecs->table('agent_shop') . " SET `user_id`= " . $_SESSION['user_id'] . ", `chuzu_status` = 1, `chuzu_time` = '" . gmtime() . "', `end_time` = '" . $end_time . "' WHERE `shop_id`= " . $shop_id;
		$db->query($sql);
		...
		show_message('店主账号创建失败', $_LANG['back_page_up'], '', 'error');
	}
}
...
function action_agent_cx_shop() {
	...
	$supplier_id = intval($_REQUEST['shop_id']);	//店铺ID
	...
	if($supplier_id > 0){
        ...
        $update_info = array(
            ...
            'update ' . $ecs->table('agent_shop') . ' set user_id = 0, chuzu_status = 0 WHERE shop_id = ' . $supplier_id
        );
    ...
    show_message('撤销成功！', '返回上一页', 'user.php?act=agent_shop_list&chuzu_status=1');
}

/**
 * 我的店铺管理
 */
function action_shop_index() {
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	$smarty = $GLOBALS['smarty'];
	...
	$sql = "SELECT value FROM " . $ecs->table('supplier_shop_config') . " WHERE code = 'shop_name' AND supplier_id = " . $shop_info['shop_id'];
	$shop_name = $db->getOne($sql);	//店铺名称

	$smarty->assign('shop_id', $shop_info['shop_id']);
	$smarty->assign('shop_name', $shop_name);
	$smarty->assign('shop_code', $shop_info['shop_code']);

	// 2018 wejun start  查询推荐抽奖商品
	$sql = 'SELECT eg.goods_id, g.goods_name,  eg.exchange_integral, ' .
		' g.goods_thumb , g.goods_img ' .
		'FROM ' . $GLOBALS['ecs']->table('exchange_goods') . ' AS eg, ' .$GLOBALS['ecs']->table('goods') . ' AS g ' .
		"WHERE eg.goods_id = g.goods_id AND eg.is_best = 1";

	$exchange_goods = $db->getRow($sql);
	// print_r($exchange_goods);exit;
	$smarty->assign('exchange_goods', $exchange_goods);
	$smarty->assign('supplier_id', $shop_info['shop_id']);		//记录店铺id
	// 2018.01.31 wejun end


    $smarty->display('shop_index.dwt');
}

/**
 * 店铺租约
 */
function action_shop_zuyue() {
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	$smarty = $GLOBALS['smarty'];
	...
	// 判断店铺是否已到期;
	if(gmtime() > $shop_info['end_time']) {
	    // 已过期
	    $sy_time = '已过期';
	} else {
		$sy_time = ShengYu_Tian_Shi_Fen($shop_info['end_time']);
	}
	...
	$smarty->assign('shop_name', $shop_name);
	$smarty->assign('shop_code', $shop_info['shop_code']);
	$smarty->assign('sy_time', $sy_time);
	$smarty->assign('chuzu_time', local_date('Y-m-d', $shop_info['chuzu_time']));
	$smarty->assign('end_time', local_date('Y-m-d', $shop_info['end_time']));
	$smarty->assign('shop_renter_cost', $_CFG['shop_renter_cost']);

    $smarty->display('shop_zuyue.dwt');
}

/**
 * 开店申请说明
 */
function action_openshop_index() {
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	$smarty = $GLOBALS['smarty'];
	...
	$sql = "SELECT status,supplier_remark FROM " . $ecs->table('supplier') . " WHERE user_id = '$user_id'";
	$supplier = $db->getRow($sql);	//店铺名称
	if($supplier){
		if($supplier['status']==0){
			//审核中
			show_message('亲，您的资料已经提交，请耐心等候。');
			exit;
		}elseif($supplier['status']==-1&&$_GET['again']!=1){
			//审核拒绝
			show_message($supplier['supplier_remark'],'重新填写','user.php?act=openshop_index&again=1');
			exit;
		}elseif($supplier['status']==1){
			//审核拒绝
			show_message('您的店铺已经通过审核','前往管理','user.php?act=shop_index');
			exit;
		}
	}

    $smarty->assign('shop_renter_cost', $_CFG['shop_renter_cost']);
    $smarty->display('openshop_index.dwt');
}

/**
 * 开店申请
 */
function action_openshop_apply() {
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	$smarty = $GLOBALS['smarty'];
	...
	// 判断是否店主，如果已经是店主则不能申请，跳转到首页
	$sql = "SELECT * FROM " . $ecs->table('agent_shop') . " WHERE user_id = " . $user_id;
    $shop_info = $db->getRow($sql);
    if ($shop_info && $shop_info['user_id'] > 0) {
    	show_message('亲，您已经是店主了，无须申请了。');
	...
	$city_list = get_regions_wap(-1);
	$district_list = get_regions_wap(-1);
    ...
    $smarty->display('openshop_apply.dwt');
}

/**
 * 开店申请提交
 */
function action_openshop_insert() {
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	$smarty = $GLOBALS['smarty'];
	...
	$name = trim($_REQUEST['name']);	//联系人姓名
	$phone = trim($_REQUEST['phone']);	//手机号
	$country = intval($_REQUEST['country']);	//国家
	$province = intval($_REQUEST['province']);	//省
	$city = intval($_REQUEST['city']);	//市
	$district = intval($_REQUEST['district']);	//区
	$address = trim($_REQUEST['address']);	//详细地址
	$id_card_no = trim($_REQUEST['id_card_no']);	//身份证号码

	if(isset($_FILES['idcard_front']) && $_FILES['idcard_front']['tmp_name'] != '') {
		$idcard_front = $image->upload_image($_FILES['idcard_front'], 'supplier');
		if($idcard_front === false) {
			show_message($image->error_msg());
		}
	}
	if(isset($_FILES['idcard_reverse']) && $_FILES['idcard_reverse']['tmp_name'] != '') {
		$idcard_reverse = $image->upload_image($_FILES['idcard_reverse'], 'supplier');
		if($idcard_reverse === false) {
			show_message($image->error_msg());
		}
	}
	...
	/*开店申请*/
	$save['supplier_id'] = generate_id();	//生成一个agent_shop和supplier表都没有的ID
	$save['user_id'] = $_SESSION['user_id'];
	$save['company_name'] = $shop_name;
	$save['contacts_name'] = $shop_name;
	$save['contacts_phone'] = $phone;
	$save['tel'] = $phone;
	...
	$save['id_card_no'] = $id_card_no;
	$save['applynum'] = 3;
	$save['supplier_name'] = $shop_name;
	$save['idcard_front'] = $idcard_front;
	$save['idcard_reverse'] = $idcard_reverse;
	$save['supplier_rebate'] = 0;	//平台分成百分比6为6%
	$save['status'] = 0;	//默认为待审核
	$save['rank_id'] = 0;	//店铺等级
	$save['agent_id'] = 0;	//业主ID 还没有分配
	$save['shop_code'] = '';	//店铺编码 还没有
	$save['add_time'] = gmtime();
	$db->autoExecute($ecs->table('supplier'), $save);
	$supplier_id = $db->insert_id();

	if ($supplier_id) {
		$smarty->assign('page_title', '申请开店'); // 页面标题
		$smarty->display('apply_ok.dwt');
	} else {
...
function action_my_recommend() {
	...
	// 是否业主或店主
	$sql = "SELECT is_agent, shop_id FROM " . $ecs->table('users') . " WHERE user_id = " . $user_id;
	$user_info = $db->getRow($sql);

	$page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;

	// 既是店主又是业主
	if ($user_info['is_agent'] && $user_info['shop_id']) {
		$type = intval($_REQUEST['type']);

		//推荐奖励总额
		$sql = "SELECT SUM(money) FROM " . $ecs->table('agent_reward_log') . " WHERE reward_type = 3 AND agent_id = " . $user_id;
		$reward1 = $db->getOne($sql);	//推荐业主品购买获得奖励
		$reward1 = $reward1 ? $reward1 : 0;

		$sql = "SELECT SUM(money) FROM " . $ecs->table('affiliate_log') . " WHERE separate_type = 0 AND user_id = " . $user_id;
		$reward2 = $db->getOne($sql);	//注册分享商品购买奖励
		$reward2 = $reward2 ? $reward2 : 0;
		$reward_total = $reward2 + $reward2;
		$smarty->assign('reward_total', $reward_total);

		// 下级业主数
		$sql = "SELECT COUNT(*) FROM " . $ecs->table('users') . " WHERE (pid1 = " . $user_id . " OR pid2 = " . $user_id . "  OR pid3 = " . $user_id . ") AND is_agent = 1";
		$sub_agent_count = $db->getOne($sql);
		$smarty->assign('sub_agent_count', $sub_agent_count);

		// 分享推荐人数
		$sql = "SELECT COUNT(*) FROM " . $ecs->table('users') . " WHERE parent_id = " . $user_id;
		$sub_user_count = $db->getOne($sql);
		$smarty->assign('sub_user_count', $sub_user_count);
		...
		if ($type == 0) {
			...
			$sql = "SELECT user_id, user_name, agent_code, pid1, pid2, pid3, reg_time FROM " . $ecs->table('users') . " WHERE (pid1 = " . $user_id . " OR pid2 = " . $user_id . "  OR pid3 = " . $user_id . ") AND is_agent = 1 ORDER BY reg_time DESC LIMIT " . $pager['start'] . ", " . $pager['size'];
			$sub_agent_list = $db->getAll($sql);
			foreach ($sub_agent_list as $key => $value) {
			    $sub_agent_list[$key]['reg_time'] = local_date('Y-m-d', $value['reg_time']);
			    if ($value['pid1'] == $user_id) {
			    	$sub_agent_list[$key]['level'] = '一级业主';
			    } elseif ($value['pid2'] == $user_id) {
			    	$sub_agent_list[$key]['level'] = '二级业主';
			    } elseif ($value['pid3'] == $user_id) {
			    	$sub_agent_list[$key]['level'] = '三级业主';
			    }
			}

			$smarty->assign('sub_agent_list', $sub_agent_list);
		} elseif ($type == 1) {
			$sql = "SELECT COUNT(*) FROM " . $ecs->table('users') . " WHERE parent_id = " . $user_id;
			$record_count = $db->getOne($sql);
			// 分页函数
			...
			$sql = "SELECT user_id, user_name, reg_time FROM " . $ecs->table('users') . " WHERE parent_id = " . $user_id . " ORDER BY reg_time DESC LIMIT " . $pager['start'] . ", " . $pager['size'];
			$sub_user_list = $db->getAll($sql);
			foreach ($sub_user_list as $key => $value) {
			    $sub_user_list[$key]['reg_time'] = local_date('Y-m-d', $value['reg_time']);
			}
			$smarty->assign('sub_user_list', $sub_user_list);
		}
		$smarty->assign('pager', $pager);
		$smarty->display('my_recommend_'.$type.'.dwt');
	}
	// 店主
	elseif ($user_info['shop_id']) {
		$sql = "SELECT SUM(money) FROM " . $ecs->table('affiliate_log') . " WHERE separate_type = 0 AND user_id = " . $user_id;
		$reward_total = $db->getOne($sql);	//注册分享商品购买奖励
		$reward_total = $reward_total ? $reward_total : 0;
		$smarty->assign('reward_total', $reward_total);
		...
		$smarty->assign('sub_user_list', $sub_user_list);
		$smarty->assign('pager', $pager);
		$smarty->display('my_recommend_2.dwt');
	}
	// 业主
	elseif ($user_info['is_agent']) {
		$sql = "SELECT SUM(money) FROM " . $ecs->table('agent_reward_log') . " WHERE reward_type = 3 AND agent_id = " . $user_id;
		$reward_total = $db->getOne($sql);	//推荐业主品购买获得奖励
		$reward_total = $reward_total ? $reward_total : 0;
		$smarty->assign('reward_total', $reward_total);
		...
		$smarty->assign('sub_agent_list', $sub_agent_list);
		$smarty->assign('pager', $pager);
		$smarty->display('my_recommend_3.dwt');
	} else {
		ecs_header("Location: index.php\n");
		exit;
	}
...
function action_my_bonus() {
	...
	$user_info = $db->getRow($sql);
	// 既是业主又是店主 或者为店主
	if (($user_info['is_agent'] && $user_info['shop_id']) || $user_info['is_agent']) {
		$type = intval($_REQUEST['type']);

		// 总分红 = 季度消费分红+推荐区产品分红
		$sql = "SELECT SUM(consume_money) FROM " . $ecs->table('user_quarter_consume') . " WHERE user_id = " . $user_id . " AND is_pay_ok = 1 AND status = 1";
		$reward1 = $db->getOne($sql);
		$reward1 = $reward1 ? $reward1 : 0;
		$smarty->assign('$reward1', $reward1);

		$sql = "SELECT SUM(money) FROM " . $ecs->table('agent_reward_log') . " WHERE agent_id = " . $user_id . " AND (reward_type = 1 OR reward_type = 2)";
		$reward2 = $db->getOne($sql);
		$reward2 = $reward2 ? $reward2 : 0;
		$smarty->assign('$reward2', $reward2);

		$reward_total = $reward1 + $reward2;
		$reward_total = $reward_total ? $reward_total : 0;
		$smarty->assign('reward_total', $reward_total);

		// 季度消费分红
		if ($type == 0) {
			$sql = "SELECT COUNT(*) FROM " . $ecs->table('user_quarter_consume') . " WHERE user_id = " . $user_id . " AND is_pay_ok = 1 AND status = 1";
			$record_count = $db->getOne($sql);
			// 分页函数
			$pager = get_pager('user.php', array( 'act' => $action ), $record_count, $page);

			$sql = "SELECT * FROM " . $ecs->table('user_quarter_consume') . " WHERE user_id = " . $user_id . " AND is_pay_ok = 1 AND status = 1 ORDER BY pay_time DESC LIMIT " . $pager['start'] . ", " . $pager['size'];
			$reward_list = $db->getAll($sql);

			foreach ($reward_list as $key => $value) {
			    $reward_list[$key]['pay_time'] = local_date('Y-m-d', $value['pay_time']);
			    $reward_list[$key]['consume_paytime_start'] = local_date('Y-m-d', $value['consume_paytime_start']);
			    $reward_list[$key]['consume_paytime_end'] = local_date('Y-m-d', $value['consume_paytime_end']);
			}
			$smarty->assign('reward_list', $reward_list);
		}
		// 用户推荐产品到公共区和业主引荐厂家产品业主分红
		elseif ($type == 1) {
			$sql = "SELECT COUNT(*) FROM " . $ecs->table('agent_reward_log') . " WHERE agent_id = " . $user_id . " AND (reward_type = 1 OR reward_type = 2)";
			$record_count = $db->getOne($sql);
			// 分页函数
			$pager = get_pager('user.php', array( 'act' => $action ), $record_count, $page);

			$sql = "SELECT * FROM " . $ecs->table('agent_reward_log') . " WHERE agent_id = " . $user_id . " AND (reward_type = 1 OR reward_type = 2) ORDER BY add_time DESC LIMIT " . $pager['start'] . ", " . $pager['size'];
			$reward_list = $db->getAll($sql);

			foreach ($reward_list as $key => $value) {
			    $reward_list[$key]['add_time'] = local_date('Y-m-d', $value['add_time']);
			}
			$smarty->assign('reward_list', $reward_list);
		}

		$smarty->assign('pager', $pager);
		$smarty->display('my_bonus_'.$type.'.dwt');
	}
	// 业主或普通会员 季度消费分红
	else {
		// 季度消费分红
		$sql = "SELECT SUM(consume_money) FROM " . $ecs->table('user_quarter_consume') . " WHERE user_id = " . $user_id . " AND is_pay_ok = 1 AND status = 1";
		$reward_total = $db->getOne($sql);
		$reward_total = $reward_total ? $reward_total : 0;
		$smarty->assign('reward_total', $reward_total);
...
function action_my_benefits() {
	...
	if (($user_info['is_agent'] && $user_info['shop_id']) || $user_info['is_agent']) {
		...
		// 推荐抽奖
		$sql = "SELECT SUM(money) FROM " . $ecs->table('affiliate_log') . " WHERE separate_type = 3 AND user_id = " . $user_id;
		$reward2 = $db->getOne($sql);	//注册分享抽奖奖励
		$reward2 = $reward2 ? $reward2 : 0;
		$smarty->assign('reward2', $reward2);

		// 店铺租金
		$sql = "SELECT SUM(rebate_money) FROM " . $ecs->table('agent_rebate_log') . " WHERE agent_id = " . $user_id;
		$reward3 = $db->getOne($sql);
		$reward3 = $reward3 ? $reward3 : 0;
		$smarty->assign('reward3', $reward3);

		$reward_total = $reward1 + $reward2 + $reward3;
		$reward_total = $reward_total ? $reward_total : 0;
		$smarty->assign('reward_total', $reward_total);

		// 业主返利
		if ($type == 0) {
			$sql = "SELECT COUNT(*) FROM " . $ecs->table('agent_reward_log') . " WHERE reward_type = 3 AND agent_id = " . $user_id;
			$record_count = $db->getOne($sql);
			// 分页函数
			$pager = get_pager('user.php', array( 'act' => $action ), $record_count, $page);

			$sql = "SELECT * FROM " . $ecs->table('agent_reward_log') . " WHERE reward_type = 3 AND agent_id = " . $user_id . " ORDER BY add_time DESC LIMIT " . $pager['start'] . ", " . $pager['size'];
			$reward_list = $db->getAll($sql);

			foreach ($reward_list as $key => $value) {
			    $reward_list[$key]['add_time'] = local_date('Y-m-d', $value['add_time']);
			    $reward_list[$key]['source_agent_name'] = $db->getOne("SELECT user_name FROM " . $ecs->table('users') . " WHERE user_id = " . $value['source_agent_id']);
			}
		...
		// 推荐抽奖
		elseif ($type == 1) {
			$sql = "SELECT COUNT(*) FROM " . $ecs->table('affiliate_log') . " WHERE separate_type = 3 AND user_id = " . $user_id;
			$record_count = $db->getOne($sql);
			// 分页函数
			$pager = get_pager('user.php', array( 'act' => $action ), $record_count, $page);

			$sql = "SELECT * FROM " . $ecs->table('affiliate_log') . " WHERE separate_type = 3 AND user_id = " . $user_id . " ORDER BY time DESC LIMIT " . $pager['start'] . ", " . $pager['size'];
			$reward_list = $db->getAll($sql);

			foreach ($reward_list as $key => $value) {
			    $reward_list[$key]['time'] = local_date('Y-m-d', $value['add_time']);
			    $reward_list[$key]['source_user_name'] = $db->getOne("SELECT b.user_name FROM " . $ecs->table('order_info') . " a LEFT JOIN " . $ecs->table('users') . " b ON a.user_id = b.user_id WHERE a.order_id = " . $value['order_id']);
			}
		...
		// 店铺租金
		elseif ($type == 2) {
			$sql = "SELECT COUNT(*) FROM " . $ecs->table('agent_rebate_log') . " WHERE agent_id = " . $user_id;
			$record_count = $db->getOne($sql);
			// 分页函数
			$pager = get_pager('user.php', array( 'act' => $action ), $record_count, $page);

			$sql = "SELECT * FROM " . $ecs->table('agent_rebate_log') . " WHERE agent_id = " . $user_id . " ORDER BY add_time DESC LIMIT " . $pager['start'] . ", " . $pager['size'];
			$reward_list = $db->getAll($sql);

			foreach ($reward_list as $key => $value) {
			    $reward_list[$key]['add_time'] = local_date('Y-m-d', $value['add_time']);
			    $reward_list[$key]['shop_code'] = $db->getOne("SELECT shop_code FROM " . $ecs->table('agent_shop') . " WHERE shop_id = " . $value['shop_id']);
			}

			$smarty->assign('reward_list', $reward_list);
		}
		$smarty->assign('pager', $pager);
		$smarty->display('my_benefits_'.$type.'.dwt');
	}
	// 店主
	elseif ($user_info['shop_id']) {
		// 推荐抽奖
		$sql = "SELECT SUM(money) FROM " . $ecs->table('affiliate_log') . " WHERE separate_type = 3 AND user_id = " . $user_id;
		$reward_total = $db->getOne($sql);	//注册分享抽奖奖励
		$reward_total = $reward_total ? $reward_total : 0;
		$smarty->assign('reward_total', $reward_total);
		...
		$smarty->assign('reward_list', $reward_list);
		$smarty->assign('pager', $pager);
		$smarty->display('my_benefits_3.dwt');
	}else {
		ecs_header("Location: index.php\n");
		exit;
	}
...
function action_my_bank() {
    ...
    $smarty->display('my_bank.dwt');
}

/**
 * 添加银行卡
 */
function action_add_bank() {
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	$smarty = $GLOBALS['smarty'];
	...
	$sql = "SELECT * FROM " . $ecs->table('bank_list') . " ORDER BY add_time DESC";
	$bank_list = $db->getAll($sql);
	// 赋值于模板
	$smarty->assign('bank_list', $bank_list);
	$smarty->assign('source_url', basename($_SERVER['HTTP_REFERER']));//来源地址

    $smarty->display('add_bank.dwt');
}

/**
 * 添加银行卡处理
 */
function action_insert_bank() {
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	$smarty = $GLOBALS['smarty'];
	...
	$bank_num = trim($_REQUEST['bank_num']);
	$bank_branch = trim($_REQUEST['bank_branch']);
	$bank_user = trim($_REQUEST['bank_user']);
	$source_url = trim($_REQUEST['source_url']);//来源地址，用了判断跳转URL
	$add_time = gmtime();
	...
	if (empty($bank_branch)) {
		show_message('请填写开户支行');
	}
	if (empty($bank_user)) {
		show_message('请填写姓名');
	}
	if (check_bankCard($bank_num) === false) {
		show_message('银行卡号不正确');
	}

	$sql = "SELECT COUNT(*) FROM " . $ecs->table('user_bank') . " WHERE bank_num = '$bank_num'";
	$bank_exist = $db->getOne($sql);
	if ($bank_exist) {
		show_message('银行卡已存在');
	}

	$sql = "INSERT INTO " . $ecs->table('user_bank') . " (user_id,bank_id,bank_num,bank_branch,bank_user,add_time) VALUES ('$user_id','$bank_id','$bank_num','$bank_branch','$bank_user','$add_time') ";
	$res = $db->query($sql);

	if ($res) {
		if ($source_url == 'user.php?act=my_bank') {
			ecs_header("Location: user.php?act=my_bank\n");
		} elseif ($source_url == 'user.php?act=select_bank' || $source_url == 'user.php?act=account_raply') {
			ecs_header("Location: user.php?act=account_raply\n");
		}
		exit;
	} else {
		show_message('添加银行卡失败');
	}
}
...
function action_del_message() {
	...
	$sql = "DELETE FROM " . $GLOBALS['ecs']->table('message_log') . " WHERE user_id = " . $user_id . " AND msg_id = " . $message_id;
	$res = $db->query($sql);
	if ($res) {
		echo 1;
		exit;
...
function generate_id ()
{
	...
	$sql = "select count(*) from " . $GLOBALS['ecs']->table('agent_shop') . " where shop_id = '$id'";
	$count1 = $GLOBALS['db']->getOne($sql);

	$sql = "select count(*) from " . $GLOBALS['ecs']->table('supplier') . " where supplier_id = '$id'";
