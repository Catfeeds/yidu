...
if ($_GET['flag'] == 1) {
	...
	$notify_url = "http://".$_SERVER['HTTP_HOST']."/mobile/pay/ajax_url_ex.php";

} elseif ($_GET['flag'] == 2) {

	// 店铺续约支付

	$notify_url = "http://".$_SERVER['HTTP_HOST']."/mobile/pay/ajax_url_xy.php";
...
if ($_GET['flag'] == 1) {
	...
	$call_back_url = "http://".$_SERVER['HTTP_HOST']."/mobile/pay/result_url_ex.php";

} elseif ($_GET['flag'] == 2) {

	// 店铺续约支付

	$call_back_url = "http://".$_SERVER['HTTP_HOST']."/mobile/pay/result_url_xy.php";
...
if ($_GET['flag'] != 2) { //wenjun ,续约不是这个判断的
if($order_amount != $total_fee ){
	...
	// $sql = "select amount from ".$GLOBALS['ecs']->table('shop_xuyue_log') . " where log_id = '$out_trade_no'";
