...

{
	...
	if(isset($order['shipping_status']) && $order['shipping_status'] == SS_RECEIVED && get_cod_id($order_id)){
        ...
        get_rebate_from_self($order_id);



        // 用户购买推荐区商品，业主下级店主商品奖励

        get_reward_from_user($order_id, 1);

        // 业主引荐厂家商品奖励

        get_reward_from_user($order_id, 2);

        // 旧业主介绍新业主三级分销佣金

        get_reward_from_user($order_id, 3);
 ...
 * 用户消费返利，(订单收货确认后触发)henson

 */

function get_rebate_from_self($order_id)

{
    ...
    $row = $db->getRow("SELECT a.order_sn, a.is_rebate, a.user_id, b.user_name FROM " . $ecs->table('order_info') . " a LEFT JOIN " . $ecs->table('users') . " b ON a.user_id = b.user_id WHERE order_id = '$order_id'");



    $up_uid = $row['user_id']; //返利给到的用户



    $row1 = $db->getAll("SELECT order_id,goods_number,goods_price,cost_price,promote_price FROM " . $ecs->table('order_goods')." WHERE order_id = '$order_id'");
    ...
    if (empty($row['is_rebate']))

    {

        $rebate_per = (float)$GLOBALS['_CFG']['rebate_per'];   //利润分成百分比
...

        {
            ...
            if($row1[$i]['promote_price'] == $row1[$i]['cost_price'] || $row1[$i]['cost_price'] == 0)
            ...
            $money += round($all_cost_price,2); //商品利润总额

        }



        $setmoney = round($money * $rebate_per, 2); //订单所有商品返利金额



        if ($setmoney > 0) {

            $info = '订单号 ' . $order_sn . ', 返利:金钱 ' . $setmoney;

            log_account_change($up_uid, $setmoney, 0, 0, 0, $info);

            write_user_rebate_log($order_id, $up_uid, $row['user_name'], $setmoney, $info);
...
function write_user_rebate_log($order_id, $user_id, $user_name, $money, $change_desc)

{

    $add_time = gmtime();

    $sql = "INSERT INTO " . $GLOBALS['ecs']->table('user_rebate_log') . "( order_id, user_id, user_name, add_time, money, change_desc)" . " VALUES ( '$order_id', '$user_id', '$user_name', '$add_time', '$money', '$change_desc')";
 ...
 * @param  int $reward_type （1用户购买推荐区商品，业主下级店主商品奖励 2业主引荐厂家商品奖励 3旧业主介绍新业主三级分销佣金）

 */

function get_reward_from_user($order_id, $reward_type)

{
    ...
    $row = $db->getRow("SELECT a.order_sn, a.is_reward, a.user_id, b.user_name FROM " . $ecs->table('order_info') . " a LEFT JOIN " . $ecs->table('users') . " b ON a.user_id = b.user_id WHERE order_id = '$order_id'");



    $row1 = $db->getAll("SELECT a.order_id,a.goods_id,a.goods_number,a.goods_price,a.cost_price,a.promote_price,b.is_best FROM " . $ecs->table('order_goods') . " a LEFT JOIN " . $ecs->table('goods') . " b ON a.goods_id = b.goods_id WHERE is_best = 1 AND order_id = '$order_id'");
    ...
    if (empty($row['is_reward']))

    {

        if ($reward_type == 1 || $reward_type == 2) {

            $recommend_rebate_per = (float)$GLOBALS['_CFG']['recommend_rebate_per'];   //推荐区产品利润分成百分比

            if ($recommend_rebate_per) {

                $recommend_rebate_per /= 100;

            }

        } elseif ($reward_type == 3) {

            $reward_level_1 = (float)$GLOBALS['_CFG']['reward_level_1'];   //一级业主奖励
            ...
            $reward_level_2 = (float)$GLOBALS['_CFG']['reward_level_2'];   //二级业主奖励
            ...
            $reward_level_3 = (float)$GLOBALS['_CFG']['reward_level_3'];   //三级业主奖励
...

        {
            ...
            $all_goods_price  = $row1[$i]['goods_price']  * $row1[$i]['goods_number'];  //该商品总金额
            ...
            $money = round($all_cost_price,2); //每个商品的总利润



            if ($reward_type == 1) {

                $setmoney = round($money * $recommend_rebate_per, 2); //该商品奖励总额



                // 奖励的业主信息

                $sql = "SELECT a.supplier_id, a.goods_name, b.agent_id, c.user_name AS agent_name FROM " . $ecs->table('goods') . " a LEFT JOIN " . $ecs->table('supplier') . " b ON a.supplier_id = b.supplier_id LEFT JOIN " . $ecs->table('users') . " c ON b.agent_id = c.user_id WHERE a.goods_id = " . $row1[$i]['goods_id'];

                $agent_info = $db->getRow($sql);

                $up_uid = $agent_info['agent_id'];  // 奖励的业主ID

                if (!empty($up_uid) && $setmoney > 0) {

                    $info = '推荐区下级店主商品ID ' . $row1[$i]['goods_id'] . ', 奖励:金钱 ' . $setmoney;

                    $change_desc = '推荐区下级店主商品奖励';

                    log_account_change($up_uid, $setmoney, 0, 0, 0, $info);

                    write_agent_reward_log($order_id, $row1[$i]['goods_id'], $up_uid, $agent_info['agent_name'], $setmoney, $reward_type, $change_desc, 0, $all_goods_price);
                ...
                $sql = "SELECT a.factory_id, b.agent_id, c.user_name AS agent_name FROM " . $ecs->table('goods') . " a LEFT JOIN " . $ecs->table('factory') . " b ON a.factory_id = b.factory_id LEFT JOIN " . $ecs->table('users') . " c ON b.agent_id = c.user_id WHERE a.goods_id = " . $row1[$i]['goods_id'];
                ...
                if (!empty($up_uid) && $setmoney > 0) {
                    ...
                    $info = '推荐区厂家引荐商品ID ' . $row1[$i]['goods_id'] . ', 奖励:金钱 ' . $setmoney;

                    $change_desc = '推荐区厂家引荐商品奖励';
                ...
                $sql = "SELECT a.supplier_id, a.goods_name, b.agent_id, c.user_name AS agent_name, c.agent_code FROM " . $ecs->table('goods') . " a LEFT JOIN " . $ecs->table('supplier') . " b ON a.supplier_id = b.supplier_id LEFT JOIN " . $ecs->table('users') . " c ON b.agent_id = c.user_id WHERE a.goods_id = " . $row1[$i]['goods_id'];

                $agent_info = $db->getRow($sql);

                if (!empty($agent_info['agent_id'])) {

                    $pids = get_parent_uid($agent_info['agent_id']);
                    ...
                    $setmoney_1 = round($money * $reward_level_1, 2);

                    if (!empty($pid1) && $setmoney_1 > 0) {

                        $info = '推荐业主 ' . $agent_info['agent_code'] . ', 奖励:金钱 ' . $setmoney_1;

                        $change_desc = '推荐一级业主奖励';

                        log_account_change($pid1, $setmoney_1, 0, 0, 0, $info);

                        write_agent_reward_log($order_id, $row1[$i]['goods_id'], $pid1, $agent_info['agent_name'], $setmoney_1, $reward_type, $change_desc, $agent_info['agent_id'], $all_goods_price);
                    ...
                    $setmoney_2 = round($money * $reward_level_2, 2);

                    if (!empty($pid2) && $setmoney_2 > 0) {

                        $info = '推荐业主 ' . $agent_info['agent_code'] . ', 奖励:金钱 ' . $setmoney_2;

                        $change_desc = '推荐二级业主奖励';

                        log_account_change($pid2, $setmoney_2, 0, 0, 0, $info);

                        write_agent_reward_log($order_id, $row1[$i]['goods_id'], $pid2, $agent_info['agent_name'], $setmoney_2, $reward_type, $change_desc, $agent_info['agent_id'], $all_goods_price);
                    ...
                    $setmoney_3 = round($money * $reward_level_3, 2);

                    if (!empty($pid3) && $setmoney_3 > 0) {

                        $info = '推荐业主 ' . $agent_info['agent_code'] . ', 奖励:金钱 ' . $setmoney_3;

                        $change_desc = '推荐三级业主奖励';

                        log_account_change($pid3, $setmoney_3, 0, 0, 0, $info);

                        write_agent_reward_log($order_id, $row1[$i]['goods_id'], $pid3, $agent_info['agent_name'], $setmoney_3, $reward_type, $change_desc, $agent_info['agent_id'], $all_goods_price);
...
function write_agent_reward_log($order_id, $goods_id, $agent_id, $agent_name, $money, $reward_type, $change_desc, $source_agent_id, $all_goods_price)

{

    $add_time = gmtime();

    $sql = "INSERT INTO " . $GLOBALS['ecs']->table('agent_reward_log') . "( order_id, goods_id, agent_id, agent_name, add_time, money, reward_type, change_desc, source_agent_id, goods_price)" . " VALUES ( '$order_id', '$goods_id', '$agent_id', '$agent_name', '$add_time', '$money', '$reward_type', '$change_desc', '$source_agent_id', '$all_goods_price')";
...

{
    ...
    $sql = "SELECT g.goods_name, g.goods_sn, g.is_on_sale, g.is_real, ".
                ...
                "g.promote_end_date, g.goods_weight, g.integral, g.extension_code, g.is_best, ".
    ...
    $parent = array(
        ...
        'is_recommend'      => $goods['is_best']    //是否推荐区商品   下单后修改为非推荐区商品不影响奖励
...

    {
        ...
        $sql = "SELECT a.goods_number, b.is_best FROM " .$GLOBALS['ecs']->table('cart').

                " a LEFT JOIN " . $GLOBALS['ecs']->table('goods') . " b ON a.goods_id = b.goods_id".

                " WHERE a.session_id = '" .SESS_ID. "' AND a.goods_id = '$goods_id' ".

                " AND a.parent_id = 0 AND a.goods_attr = '" .get_goods_attr_info($spec). "' " .

                " AND a.user_id= '".$_SESSION['user_id']."'".

                " AND a.rec_type = 'CART_GENERAL_GOODS'";
...
function stock_right_to_give($order)

{

    $sql = "SELECT SUM(og.goods_number * g.stock_right) AS stock_right " .
 ...
 * 获取订单对应季度的消费记录id(所有购物会员都有)

 * @param int $user_id  会员id

 */

function get_quarter_consume($user_id){

    $spkey = intval($user_id);
    ...
    $sql = "select consume_id, consume_paytime_start, consume_paytime_end from ". $GLOBALS['ecs']->table('user_quarter_consume') ." where user_id='$spkey' and is_pay_ok=0 order by consume_id desc limit 0,1";
    ...
    if (  $nowtime >=  $row['consume_paytime_start']  && $nowtime <= $row['consume_paytime_end'] )
...

    {
...

        {
            ...
            insert_id_consume($spkey);

            $sql2 = "select consume_id, consume_paytime_start, consume_paytime_end from ". $GLOBALS['ecs']->table('user_quarter_consume') ." where user_id='$spkey' and is_pay_ok=0 order by consume_id desc limit 0,1";

            $row2 = $GLOBALS['db']->getRow($sql2);

            if (  $nowtime >=  $row2['consume_paytime_start']  && $nowtime <= $row2['consume_paytime_end'] )

            {

                $consume_id= $row2['consume_id'];
...
function insert_id_consume($user_id)

{

    $sql = "select consume_paytime_start, consume_paytime_end from ". $GLOBALS['ecs']->table('user_quarter_consume') ." where user_id= '$user_id' and is_pay_ok=0 order by consume_id DESC LIMIT 0,1";

    $row = $GLOBALS['db']->getRow($sql);

    if (!$row['consume_paytime_start'])

    {

        $consume_paytime_start = local_mktime(0,0,0,local_date('m'), local_date('d'), local_date('Y'));

    }

    if (!$row['consume_paytime_end'])

    {
...

        {
            ...
            $consume_paytime_end= local_mktime(23,59,59,3,31,local_date("Y"));
...

        {
            ...
            $consume_paytime_end= local_mktime(23,59,59, 6,30,local_date("Y"));
...

        {
            ...
            $consume_paytime_end= local_mktime(23,59,59, 9, 30,local_date("Y"));
...

        {
            ...
            $consume_paytime_end= local_mktime(23,59,59, 12,31,local_date("Y"));

        }

    }

    if ( $row['consume_paytime_start']  &&  $row['consume_paytime_end'] )

    {

        $consume_paytime_start = $row['consume_paytime_end'] + 1;

        if (local_date("m",$consume_paytime_start)=='1' || local_date("m")=='2' || local_date("m")=='3')
    ...
    $sql="insert into ". $GLOBALS['ecs']->table('user_quarter_consume') ."(consume_paytime_start, consume_paytime_end, user_id) value('$consume_paytime_start', '$consume_paytime_end', '$user_id') ";
