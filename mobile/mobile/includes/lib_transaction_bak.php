...

{
    ...
    //wenjun start                   查询sql添加了is_lucky这个字段

    $sql = "SELECT order_id, order_sn, order_status, shipping_status, pay_status, add_time, shipping_time,is_lucky " .
...

{
...

    {
...

        {
...

            {
...

                {
                    ...
                    if ($row['status_back'] > 0) {

                        @$row['handler'] .= "<a href='javascript:void(0)'>已申请退款</a>";

                    } else {

                        @$row['handler'] .= "<a href='user.php?act=back_order&order_id=$row[order_id]&order_all=1&x=1'>退款</a>";
        ...
        if($row['order_status']==1 && $row['shipping_status']==0 && $row['pay_status']==2){

                $row['order_2017status']='进行中';

        }

        if($row['order_status']==2 && $row['shipping_status']==0 && $row['pay_status']==0){

                $row['order_2017status']='已取消';

        }

        if($row['order_status']==0 && $row['shipping_status']==0 && $row['pay_status']==0){

                $row['order_2017status']='未付款';

        }

        if($row['order_status']==3){

                $row['order_2017status']='未中奖';

        }

        if($row['order_status']==1 && $row['shipping_status']==0 && $row['pay_status']==2 && $row['is_lucky']==1){

                $row['order_2017status']='已中奖';
        ...
        if ($row['extension_code']=='exchange_goods') {
            ...
            $exchange_lucky = $GLOBALS['db']->getOne("SELECT exchange_lucky FROM " . $GLOBALS['ecs']->table('exchange_goods') . " WHERE goods_id = " . $row['extension_id']);
		...
		$arr[$row['order_id']] = array(
            ...
            'order_2017status'=>$row['order_2017status'],

            'order_extension_num'=>$row['extension_num'],

            'order_exchange_lucky' =>$exchange_lucky,
...
function get_shop_orders($supplier_id, $where='',$page,$action)

{
    ...
    $count_sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('order_info') . ' as o '.

           " LEFT JOIN " .$GLOBALS['ecs']->table('supplier_shop_config') . 'as ssc '.

           " ON o.supplier_id=ssc.supplier_id AND ssc.code='shop_name' ".

           " LEFT JOIN " .$GLOBALS['ecs']->table('payment') . 'as pm '.

           " ON o.pay_id=pm.pay_id ".

           " $where";

    $record_count = $GLOBALS['db']->getOne($count_sql);
    ...
    $sql = "SELECT o.*, ssc.value as shopname, pm.pay_code, " .
...

    {
...

        {
            ...
            if(in_array($row['pay_id'],payment_id_list(true))){
                ...
                // @$row['handler'] = "<a style='background:#E71F19; color:#fff; border:1px solid #E71F19' href=\"shop.php?act=order_detail&order_id=" .$row['order_id']. '">' .$GLOBALS['_LANG']['pay_money']. '</a>';
...

        {
...

            {
...

                {
                    ...
                    @$row['handler'] = "<a style='background:#E71F19; color:#fff; border:1px solid #E71F19'  href=\"shop.php?act=order_detail&order_id=" .$row['order_id']. '">' .$GLOBALS['_LANG']['view_order']. '</a>';
...

                {
                    ...
                    @$row['handler'] .= "<a style=\"background:#E71F19; color:#fff; border:1px solid #E71F19\" href=\"shop.php?act=shipping_index&order_id=" .$row['order_id']. '">发货</a>';

                    if ($row['status_refund'] >= 0) {

                        @$row['handler'] .= "<a href='javascript:void(0)' style='margin-left:5px'>已申请退款</a>";
...
function get_shop_back_orders($supplier_id, $where='',$page,$action)

{
    ...
    $sql = "SELECT o.*, ssc.value as shopname, bo.back_id, bo.status_back, pm.pay_code, " .
...

        {
...

            {
...

                {
                    ...
                    @$row['handler'] .= "<a style=\"background:#E71F19; color:#fff; border:1px solid #E71F19\" href=\"shop.php?act=back_detail&back_id=" .$row['back_id']. '">退款详情</a>&nbsp;';

                    @$row['handler'] .= "<a style=\"background:#E71F19; color:#fff; border:1px solid #E71F19\" href=\"shop.php?act=back_refund&back_id=" .$row['back_id']. '">确认退款</a>&nbsp;';
...
function cancel_shop_order($order_id, $supplier_id = 0)

{

    /* 查询订单信息，检查状态 */

    $sql = "SELECT user_id, order_id, order_sn , surplus , integral , bonus_id, order_status, shipping_status, pay_status, supplier_id FROM " .$GLOBALS['ecs']->table('order_info') ." WHERE order_id = '$order_id'";
    ...
    if ($supplier_id > 0 && $order['supplier_id'] != $supplier_id)
...
function cancel_order($order_id, $user_id = 0, $ex = 0)
...

        {
            ...
            if($order['pay_name'] == '支付宝'){
                ...
                if ($GLOBALS['is_app'] == 1) {
                    ...
                    $order['pay_online'] = '<div style="text-align:center"><input type="button" onclick="window.location.href=\'./apppay/pay.php?out_trade_no='.$order['log_id'].'&pay_code=alipay&flag=0\'"/></div>';
            ...
            if($order['pay_name'] == '微信支付'){
                ...
                if ($GLOBALS['is_app'] == 1) {
                    ...
                    $order['pay_online'] = '<div style="text-align:center"><input type="button" onclick="window.location.href=\'./apppay/pay.php?out_trade_no='.$order['log_id'].'&pay_code=weixin&flag=0\'"/></div>';
...
function get_shop_order_detail($order_id, $supplier_id = 0)

{
    ...
    if ($supplier_id > 0 && $supplier_id != $order['supplier_id'])
...
function get_ex_order_handler($order){
    ...
    if ($order['order_status'] == OS_UNCONFIRMED)
        ...
        if(in_array($order['pay_id'],payment_id_list(true))){
            ...
            $handler['handler'] .= " <a href=\"user.php?act=cancel_order&ex=1&order_id=" .$order['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\">".$GLOBALS['_LANG']['cancel']."</a>";
...
//
                ...
                $handler['handler'] .= " <a href=\"user.php?act=affirm_received&ex=1&order_id=" .$order['order_id']. "\" onclick=\"if (!confirm('".$back_info.$GLOBALS['_LANG']['confirm_received']."')) return false;\">".$GLOBALS['_LANG']['received']."</a>";
...
function get_shop_order_handler($order){
    ...
    if(in_array($order['pay_id'],payment_id_list(true))){
        ...
        $handler['handler'] .= " <a href=\"shop.php?act=cancel_order&order_id=" .$order['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\">".$GLOBALS['_LANG']['cancel']."</a>";
...

    {
...

       {
...

        {
            ...
            $handler['handler'] .= " <a href='shop.php?act=shipping_index&order_id=$order[order_id]'>发货</a>";
