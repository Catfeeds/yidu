...
class weixin {
	...
	function get_code($order, $payment) {
		...
		if ($order['flag'] == 2) {
			$return_url = 'http://' . $_SERVER ['HTTP_HOST'].'/mobile/respond_xy.php';
		} else {
		...
		//wenjun start 05-17
		if (strpos ( $_SERVER ['QUERY_STRING'], 'act=ex_order_detail' ) !== false) {
			return $this->natpayHtml ( $order );
		}
		//wenjun end
	...
	function respond() {
		...
		error_log(print_r($xml,1),3,dirname(__FILE__) . '/data/aa6.log');
		$notify->saveData ( $xml );
	...
	function respond_xy() {
		...
		if ($notify->checkSign () == TRUE) {
			if ($notify->data ["return_code"] == "FAIL") {
				...
				if (! check_money1 ( $log_id, $notify->data ['total_fee']/100 )) {
					$this->addLog ( $notify, 404 );
					return true;
				}
