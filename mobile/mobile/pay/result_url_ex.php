...
if($verify_result) {//验证成功
    ...
    if($result=="success"){
        ...
        if($_CFG['sms_user_money_change'] == 1)
        {
            ...
            // $content = sprintf(' 您获得的抽奖码是【%s】。如有疑问，请联系商城客服。【%s】',$order['extension_num'],$_CFG['sms_sign']);
...
//49 start
            ...
            $d_sql = "SELECT o.extension_id ".
                ...
                " LEFT JOIN " .$GLOBALS['ecs']->table('pay_log') . 'as p '.
                " ON o.order_id=p.order_id ".
                " WHERE p.log_id =$out_trade_no and p.is_paid=1 and o.extension_code = extension_goods";
            $extension_id=$GLOBALS['db']->getRow($d_sql);
            if ($extension_id) {
                //获取订单号
                $sql = "SELECT o.user_id,g.goods_number,o.order_id ".
                    " FROM " .$GLOBALS['ecs']->table('order_info') . ' as o '.
                ...
                if ($total*1 == 4) {
                    ...
                    foreach ($u as $k => $v) {
                        ...
                        $content = sprintf(' 您获得的抽奖码是【%s】。如有疑问，请联系商城客服。【%s】',$v['mm'],$_CFG['sms_sign']);
                        if($v['mobile_phone'])
                        {
                            require_once (ROOT_PATH . 'sms/sms.php');
