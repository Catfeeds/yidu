...
if ($_REQUEST['act'] == 'list')
{
	...
	$smarty->assign('action_link',  array('text' => '添加菜单', 'href'=>'mobile_menu.php?act=add'));
	$smarty->display('mobile_menu_list.htm');

}
elseif($_REQUEST['act'] == 'query')
{
    /* 权限判断 */
    check_authz_json('mobile_menu');
	$menu_list = get_all_menu();
	...
	make_json_result($smarty->fetch('mobile_menu_list.htm'), '',array('filter' => $ad_list['filter'], 'page_count' => $ad_list['page_count']));
}
elseif($_REQUEST['act'] == 'edit')
{
	...
	$smarty->assign('action_link',  array('text' => '菜单列表', 'href'=>'mobile_menu.php?act=list'));
	$smarty->display('mobile_menu_info.htm');
}
