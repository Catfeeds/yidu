...
require_once(ROOT_PATH . 'languages/' .$_CFG['lang']. '/admin/mads.php');
...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('action_link', array('text' => $_LANG['position_add'], 'href' => 'mad_position.php?act=add'));
    ...
    assign_query_info();
    $smarty->display('mad_position_list.htm');
}
...
{
    admin_priv('mad_position');

    /* 模板赋值 */
    $smarty->assign('ur_here',     $_LANG['position_add']);
    $smarty->assign('form_act',    'insert');

    $smarty->assign('action_link', array('href' => 'mad_position.php?act=list', 'text' => $_LANG['ad_position']));
    $smarty->assign('posit_arr',   array('position_style' =>'{foreach from=$ads item=ad}' ."\n". '{$ad}' ."\n". '{/foreach}'));

    assign_query_info();
    $smarty->display('mad_position_info.htm');
}
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if ($exc->num("position_name", $position_name) == 0)
    {
        ...
        $link[0]['href'] = 'mads.php?act=add';

        $link[1]['text'] = $_LANG['continue_add_position'];
        $link[1]['href'] = 'mad_position.php?act=add';

        $link[2]['text'] = $_LANG['back_position_list'];
        $link[2]['href'] = 'mad_position.php?act=list';
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if ($db->getOne($sql) == 0)
    {
        ...
        if ($db->query($sql))
        {
           ...
           /* 提示信息 */
           $link[] = array('text' => $_LANG['back_position_list'], 'href' => 'mad_position.php?act=list');
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('mad_position_list.htm'), '',
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $url = 'mad_position.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
