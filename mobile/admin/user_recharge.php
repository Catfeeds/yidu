 ...
 * ============================================================================
 * $Author: liubo $
 * $Id: user_recharge.php 17217 2011-01-19 06:29:08Z liubo $
*/
...
{
    /* 权限判断 */
    admin_priv('user_account_recharge');
    ...
    $smarty->assign('ur_here',       '购物币充值');
    $smarty->assign('id',            $user_id);
    $smarty->assign('payment_list',  $payment);
    $smarty->assign('action_link',   array('text' => '购物币充值', 'href'=>'user_recharge.php?act=add'));
    ...
    assign_query_info();
    $smarty->display('user_account_recharge_list.htm');
}
...
{
    admin_priv('user_account_recharge'); //权限判断
    ...
    if ($_REQUEST['act'] == 'add')
    {
        $href = 'user_recharge.php?act=list';
    }
    else
    {
        $href = 'user_recharge.php?act=list';
    }
    $smarty->assign('action_link', array('href' => $href, 'text' => '购物币充值'));

    assign_query_info();
    $smarty->display('user_account_recharge_info.htm');
}
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    $link[0]['text'] = '返回购物币充值列表';
    $link[0]['href'] = $href;

    $link[1]['text'] = '继续充值';
    $link[1]['href'] = 'user_recharge.php?act=add';
...
elseif ($_REQUEST['act'] == 'check')
{
    ...
    if ($id == 0)
    {
        ecs_header("Location: user_recharge.php?act=list\n");
        exit;
    }
...
elseif ($_REQUEST['act'] == 'action')
{
    ...
    if ($account['is_paid'] == 0)
    {
        if ($is_paid == '1' && $account['process_type'] == '0')
        {
            ...
            //更新会员余额数量
            log_account_change($account['user_id'], 0, 0, 0, $amount, '购物币充值', ACT_SAVING);
        ...
        /* 提示信息 */
        $link[0]['text'] = '返回购物币充值列表';
        $link[0]['href'] = 'user_recharge.php?act=list';
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('user_account_recharge_list.htm'), '', array('filter' => $list['filter'], 'page_count' => $list['page_count']));
}
/*------------------------------------------------------ */
//-- ajax删除一条信息
/*------------------------------------------------------ */
...
{
    /* 检查权限 */
    check_authz_json('user_account_recharge');
    ...
    if ($db->query($sql, 'SILENT'))
    {
       ...
       $url = 'user_recharge.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
//                   $GLOBALS['ecs']->table('users') . " AS u " . $where;
        ...
        /* 查询数据 */
        $sql  = 'SELECT ua.*, u.user_name, u.real_name FROM ' .
