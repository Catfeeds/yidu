...
//    " WHERE supplier_id=0  " . order_query_sql('await_pay'));
    ...
    $order['new_repay'] = $db->getOne('SELECT COUNT(*) FROM '.$ecs->table('back_order').' WHERE status_back < 6 AND status_back != 3 AND back_type = 4 AND supplier_id=0');
    //退货申请
    ...
    if(!empty($good_category))
    {
        ...
        if(!empty($good_name))
        {
            ...
            if ($_FILES['goods_img']['tmp_name'] != '' && $_FILES['goods_img']['tmp_name'] != 'none')
            {
                ...
                $goods_img_data = filter_var_array($_FILES['goods_img'], FILTER_SANITIZE_SPECIAL_CHARS);
                $original_img   = $image->upload_image($goods_img_data); // 原始图片
