...
function action_update ()
{
	...
	if (!empty($is_agent)) {
		...
		$agent_exist = $db->getOne("SELECT COUNT(*) FROM " . $ecs->table('users') . " WHERE user_name <> '" . $username . "' AND agent_code = '" . $agent_code . "'");
		if ($agent_exist > 0) {
	...
	if(isset($_FILES['face_card']) && $_FILES['face_card']['tmp_name'] != '')
	{
		$old_face_card = $db->getOne("SELECT face_card FROM " . $ecs->table('users') . " WHERE user_name = '$username'");
		$face_card_arr = filter_var_array($_FILES['face_card'], FILTER_SANITIZE_SPECIAL_CHARS);
		$face_card = $GLOBALS['image']->upload_image($face_card_arr, 'cardimg/' . date('Ym'));
		if($face_card === false)
		{
			sys_msg($GLOBALS['image']->error_msg(), 1, array(), false);
		}
	...
	if(isset($_FILES['back_card']) && $_FILES['back_card']['tmp_name'] != '')
	{
		$old_back_card = $db->getOne("SELECT back_card FROM " . $ecs->table('users') . " WHERE user_name = '$username'");
		$back_card_arr = filter_var_array($_FILES['back_card'], FILTER_SANITIZE_SPECIAL_CHARS);
		$back_card = $GLOBALS['image']->upload_image($back_card_arr, 'cardimg/' . date('Ym'));
		if($back_card === false)
		{
			sys_msg($GLOBALS['image']->error_msg(), 1, array(), false);
		}
	...
	$sql = "update " . $ecs->table('users') . " set `real_name`='$real_name',`card`='$card',`country`='$country',`province`='$province',`city`='$city',`district`='$district',`address`='$address',`status`='$status',`agent_code`='$agent_code' where user_name = '" . $username . "'";
	$db->query($sql);

	if($face_card != '')
	{
		@unlink(ROOT_PATH.$old_face_card);//删除旧身份证正面图片
		$sql = "update " . $ecs->table('users') . " set `face_card` = '$face_card' where user_name = '" . $username . "'";
		$db->query($sql);
	}
	if($back_card != '')
	{
		@unlink(ROOT_PATH.$old_back_card);//删除旧身份证反面图片
		$sql = "update " . $ecs->table('users') . " set `back_card` = '$back_card' where user_name = '" . $username . "'";
		$db->query($sql);
	}
	/* 更新用户扩展字段的数据 */
...
function user_list ()
{
	...
	if($result === false)
	{
		...
		if($filter['user_type'])
		{
			switch ($filter['user_type']) {
				case '1':	//普通会员
					$ex_where .= " AND is_agent = 0 AND shop_id = 0 ";
					break;
				case '2':	//店主
					$ex_where .= " AND shop_id > 0 ";
					break;
				case '3':	//业主
					$ex_where .= " AND is_agent = 1 ";
					break;
				default:
					break;
			}
		...
		// reg_time ".
		$sql = "SELECT user_id, user_name, email, mobile_phone, is_validated, validated, user_money, frozen_money, rank_points, pay_points, status, reg_time, froms, is_agent, shop_id, agent_code, agent_type ".
	...
	for($i = 0; $i < $count; $i ++)
	{
		...
		switch ($user_list[$i]['agent_type']) {
			case '1':
				$user_list[$i]['formated_agent_type'] = '小型业主';
				break;
			case '2':
				$user_list[$i]['formated_agent_type'] = '中型业主';
				break;
			case '3':
				$user_list[$i]['formated_agent_type'] = '大型业主';
				break;
			case '4':
				$user_list[$i]['formated_agent_type'] = 'VIP业主';
				break;
			case '5':
				$user_list[$i]['formated_agent_type'] = '创业型业主';
				break;
			default:
				# code...
				break;
