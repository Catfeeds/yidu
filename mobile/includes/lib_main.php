...

{
    ...
    $sql  = 'SELECT u.user_id, u.email, u.user_name, u.user_money, u.pay_points, u.reg_time, u.agent_code'.
...

{
    ...
    $tmp_arr_hunuo_com = explode('.', $tmp);

    $ext = end($tmp_arr_hunuo_com);
...

{
...

    {
        ...
        $license = '<a href="http://www.hunuo.com/license.php?product=ecshop_b2c&url=' . urlencode($host) . '" target="_blank"
 ...
 * @param $back_url 如果存在返回界面url，则会跳转到指定返回url，否则直接跳转到jump_url

 */

function process_transfer($jump_url,$back_url = ''){

    if(!empty($back_url)){

        ecs_header('Location:' . $back_url);

    }else if(!empty($jump_url)){

        ecs_header('Location:' . $jump_url);

    }else{

        ecs_header('Location:javascript:history.back()');
