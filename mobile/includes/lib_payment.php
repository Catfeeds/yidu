...
function return_url_xy($code)
{
    return $GLOBALS['ecs']->url() . 'respond_xy.php?code=' . $code;
}
 ...
 * @return  true
 */
function check_money1($log_id, $money)
{
    if(is_numeric($log_id))
    {
        $sql = 'SELECT amount FROM ' . $GLOBALS['ecs']->table('shop_xuyue_log') .
            " WHERE log_id = '$log_id'";
        $amount = $GLOBALS['db']->getOne($sql);
    }
    else
