...
//分享的接口
if(!empty($_REQUEST['act']) && $_REQUEST['act'] == 'share_goods_info')
{
    ...
    if ($ex) { 
        //判断是否被取消为普通商品
        $ex_sql = 'select goods_id from ' . $GLOBALS['ecs']->table('exchange_goods') . ' where goods_id=' . $goods_id;
        $count = $GLOBALS['db']->getOne($ex_sql);
        if ($count) {
            $url = "http://www.ed163.cn/mobile/goods.php?id=".$goods_id."&ex=1";
        }else{
            $url = "http://www.ed163.cn/mobile/goods.php?id=".$goods_id;
        }
    }else{
        $url = "http://www.ed163.cn/mobile/goods.php?id=".$goods_id;
    }

    $sql = 'select goods_name,goods_brief,goods_thumb from ' . $GLOBALS['ecs']->table('goods') . ' where goods_id=' . $goods_id;
    ...
    $data['goods_brief'] = $res['goods_brief'];
...
{
	$select_info = array();
	$country = remove_xss(trim($_REQUEST['country']));
    $province = remove_xss(trim($_REQUEST['province']));
    $city = remove_xss(trim($_REQUEST['city']));
...
if(!empty($_REQUEST['act']) && $_REQUEST['act'] == 'get_pickup_info')
{
	$province = remove_xss(trim($_REQUEST['province']));
    $city = remove_xss(trim($_REQUEST['city']));
    $district = remove_xss(trim($_REQUEST['district']));
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'price')
{
    ...
    else
    {
		...
		$shop_price1  = get_final_price1($goods_id, $number, true, $attr_id);
		$res['result1'] = sprintf($GLOBALS['_CFG']['currency_format'], $shop_price1 * $number);
...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
	...
	//2018 wenjun start 判断是否为抽奖商品的详情
	$ex = $_GET['ex'];

	if ($ex) {

		//每个用户进入抽奖详情的时候，都要去判断所有的抽奖订单是否有过期的订单
...
//				$arr = count(explode(',', $v['extension_num']));
		...
		$smarty->assign('ex',$ex);
		$ex_goods = get_exchange_goods_info($goods_id);
		$smarty->assign('ex_goods',$ex_goods);
		$smarty->assign('supplier_id',$_GET['supplier_id']);
	}
	//2018 wenjun end

    if ($goods['supplier_id'] > 0){
        $goods['qq'] = $db->getOne("SELECT value FROM " . $ecs->table('supplier_shop_config') . " WHERE code = 'qq' AND supplier_id = " . $goods['supplier_id']);
    } else {
        $goods['qq'] = $db->getOne("SELECT value FROM " . $ecs->table('shop_config') . " WHERE code = 'qq'");
    }
...
/* 自提点 */
 ...
 /* 自提点 End */
	...
	if ($goods['supplier_id'] > 0)
	{
		...
		$results_hunuo_com = $GLOBALS['db']->getAll($sql);
		$return_arr = array();
		foreach ($results_hunuo_com as $value)
		{
		...
		foreach ($properties['spe'] AS $skey=>$sval)
		{
			$hahaha_zhyh = 0;
			$sskey_hunuo_com = '-1';
			foreach ($sval['values'] AS $sskey=>$ssval)
			{
				if ( is_exist_prod($selected_first, $ssval['id'], $prod_exist_arr))
				{
					$hahaha_zhyh = $hahaha_zhyh ? $hahaha_zhyh : $ssval['id'];
					$sskey_hunuo_com = ($sskey_hunuo_com != '-1') ? $sskey_hunuo_com : $sskey;
				}
				else
				{
					$properties['spe'][$skey]['values'][$sskey]['disabled'] = "disabled";
				}

				if ($skey==$attr_id_gallery)
				{
					$goods_attr_id_qq = $ssval['id'] ;
					...
					$properties['spe'][$skey]['values'][$sskey]['goods_attr_thumb'] = $db->getOne($sql_qq_qq87139667);
				}
			}
			if ($hahaha_zhyh)
			{
				$selected_first[$skey] =  $hahaha_zhyh;
			}
			if ($sskey_hunuo_com!='-1')
			{
				$properties['spe'][$skey]['values'][$sskey_hunuo_com]['selected_key_hunuo'] = "1";
			}
		}
...
$db->query('UPDATE ' . $ecs->table('goods') . " SET click_count = click_count + 1 WHERE goods_id = '$id'");
