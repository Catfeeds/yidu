...
function action_getverifycode ()
{
	...
	$content = '您的验证码为' . $verifycode . '【hunuoshop】';
	/* 发送注册手机短信验证 */
...
//	$aa = $user->login($username, $password, isset($_POST['remember']));
//	print_r($aa);
//	exit();
...
function action_address_list ()
{
	...
	/* 获得用户所有的收货人信息 */
	$consignee_list = get_consignee_list_by_user_id($_SESSION['user_id']);
...
function action_act_account ()
{
	...
	{
		$surplus['defaultbank'] = isset($_POST['hunuo_bank']) ? trim($_POST['hunuo_bank']) : '';
	}
...
function action_tg_login_act ()
{
	...
	$_SESSION['takegoods_sn_hunuo'] = $tg_sn;
	$_SESSION['takegoods_id_hunuo'] = $tgrow['tg_id'];
