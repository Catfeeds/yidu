...
if($_REQUEST['act']=="upload")
{
    ...
    $array = handle_gallery_image_attr($goods_id, $goods_attr_id, $file);
...
{
	$img_id_hunuo_com = empty($_REQUEST['img_id']) ? 0 : intval($_REQUEST['img_id']);
	$sql_hunuo_com = "update ". $GLOBALS['ecs']->table('goods_gallery') ." set is_attr_image='0' where goods_id='$goods_id' and goods_attr_id='$goods_attr_id' ";
	$db->query($sql_hunuo_com);
	$sql_hunuo_com = "update ". $GLOBALS['ecs']->table('goods_gallery') ." set is_attr_image='1' where img_id='$img_id_hunuo_com' ";
	$db->query($sql_hunuo_com);
	clear_cache_files();
}
