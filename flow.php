...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'selcart')
{
	...
	if ($sel_goods)
	{
		...
		$_SESSION['sel_cartgoods'] = $sel_goods;
...
elseif ($_REQUEST['act']=='delAddress')
{
	...
	$consignee_list = get_consignee_list();
...
elseif ($_REQUEST['act']=='saveAddress')
{
	...
	$consignee = array(
            ...
            'country'       => empty($address_info->country)    ? '1' :   intval($address_info->country),
...
function get_consignee_list()
{
	...
	foreach ($consignee_list  as $cons_key => $cons_val)
	{
				...
				$consignee_list[$cons_key]['address_short_name'] .=  "<br>".(empty($cons_val['mobile']) ? $cons_val['tel'] : $cons_val['mobile']);
